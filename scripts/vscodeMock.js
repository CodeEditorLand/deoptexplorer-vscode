...
class Position {
	...
	translate(lineDelta = undefined, characterDelta = undefined) {
		const change = isObject(lineDelta)
			? lineDelta
			: { lineDelta, characterDelta };
		lineDelta = change.lineDelta ?? 0;
		characterDelta = change.characterDelta ?? 0;
		return this.with(
			Math.max(0, this.line + lineDelta),
			Math.max(0, this.character + characterDelta),
		);
	}
	/**
	 * @overload
...
class Range {
	 ...
	 */
	intersection(range) {
		if (this.start.isAfter(range.end) || this.end.isBefore(range.start)) {
			return undefined;
		}
		return this.with(
...
class Uri {
	...
	with(change) {
		...
		return this.#uri.scheme === uri.scheme &&
			...
			this.#uri.fragment === uri.fragment
			? this
			: new Uri(uri);
	}
	/**
