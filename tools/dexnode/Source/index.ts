...
if (!argv.exec_path)
	throw new Error(
		"Could not detect host. Please specify '--exec-path' with the path to the host executable",
	);
if (!argv.v8_version)
	throw new Error(
		"Could not determine V8 version. Please specify a valid version using '--v8-version'.",
	);
...
let result;
try {
	result = child_process.spawnSync(argv.exec_path, args, {
		stdio: "inherit",
	});
} finally {
