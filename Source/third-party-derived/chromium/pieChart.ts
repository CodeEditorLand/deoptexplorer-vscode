...
import { type HtmlString, html } from "#core/html.js";
...
export function pieChart(slices: readonly Slice[], options: PieOptions) {
	const cutout: Cutout | undefined = options.cutout
		? typeof options.cutout === "number"
			? {
					...defaultCutout,
					radius: Math.max(Math.min(options.cutout, 1), 0),
				}
			: typeof options.cutout === "string"
				? { ...defaultCutout, color: options.cutout }
				: typeof options.cutout === "object"
					? {
							color: options.cutout.color,
							radius: Math.max(
								Math.min(options.cutout.radius, 1),
								0,
							),
						}
					: defaultCutout
	...
	return html`
    <div class="pie-chart">
        <div class="pie-chart-root" style="${style(
			html`height: ${options.size}px; width: ${options.size}px;`,
			options.cspStyleHashesOut,
		)}">
            <svg>
            <g transform="scale(${options.size / 2}) translate(1, 1) scale(0.99, 0.99)">
                <circle r="1" stroke="hsl(0, 0%, 80%)" fill="transparent" stroke-width="${
					1 / options.size
				}"></circle>
                ${
					cutout
						? html`<circle r="${
								cutout.radius
							}" stroke="hsl(0, 0%, 80%)" fill="${
								cutout.color
							}" stroke-width="${1 / options.size}"></circle>`
						: null
				}
                ${slices.map(
					(slice) => html`
                    <path class="slice" fill="${
						slice.color
					}" d="${getPathStringForSlice(
						total,
						ref_lastAngle,
						cutout,
						slice,
					)}">
                        <title>${slice.title} &mdash; ${
							options.formatter?.(slice.value) ?? slice.value
						}</title>
                    </path>
                `,
				)}
...
function getPathStringForSlice(
	total: number,
	ref_lastAngle: Reference<number>,
