...
const sourcePositionRegExp =
	/C(?<code_offset>\d+)O(?<script_offset>\d+)(?:I(?<inlining_id>\d+))?/y;
...
export function* sourcePositionTableIterator(
	sourcePositions: string,
): Generator<SourcePositionTableEntry, void> {
	...
	while ((match = sourcePositionRegExp.exec(sourcePositions))) {
		assert(match.groups);
		const code_offset = toAddress(
			Number.parseInt(match.groups.code_offset, 10),
		);
		const script_offset = Number.parseInt(match.groups.script_offset, 10);
		const inlining_id = match.groups.inlining_id
			? Number.parseInt(match.groups.inlining_id, 10)
			: undefined;
