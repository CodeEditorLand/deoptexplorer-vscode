...
import type { DeoptimizationData } from "./deoptimizationData";
...
export class SourcePosition {
	...
	private _externalLine = 0;
	private _externalFileId = 0;
	...
	inliningStack(deopt_data: DeoptimizationData) {
		const stack: SourcePositionInfo[] = [];
		let pos: SourcePosition = this;
		while (pos.isInlined) {
			assert(pos.inliningId < deopt_data.inliningPositions.length);
			const inl = deopt_data.inliningPositions[pos.inliningId];
			const func = deopt_data.getInlinedFunction(inl.inlinedFunctionId);
