...
import {
	type FindPosition,
	type MappedPosition,
	type SourceFindPosition,
	SourceMapConsumer,
} from "source-map";
import { Location, Position, Range, type Uri } from "vscode";
...
import { UriEqualer, resolveUri } from "./uri";
...
export class SourceMap {
	...
	constructor(
		...
		this._sourceMap = new (
			source_map.SourceMapConsumer as new (
				sourceMap: source_map.RawSourceMap | string,
				sourceMapUrl?: string,
			) => source_map.SourceMapConsumer
		)(sourceMap, this._sourceMapUri.toString());
		this._sourceMap.computeColumnSpans();
	}

	get sources() {
		return (this._sources ??= this._sourceMap.sources.map((source) =>
			getCanonicalUri(pathOrUriStringToUri(source)),
		));
	}
	...
	toGeneratedRange(value: Location, bias?: SourceMapBias) {
		return this._rangeToGeneratedRange(
			value.uri.toString(),
			value.range,
			bias,
		);
	...
	forEachMapping<T>(
		cb: (this: T, mapping: SourceMapping) => void,
		thisArg: T,
		order?: MappingOrder,
	): void;
	forEachMapping(
	...
	private _positionToSourceLocation(
		position: Position,
		bias?: SourceMapBias,
	) {
	...
	private _rangeToGeneratedRange(
		source: string,
		range: Range,
		bias?: SourceMapBias,
	) {
	...
	private _positionToGeneratedRange(
		source: string,
		position: Position,
		bias?: SourceMapBias,
	) {
	...
	private _positionToGeneratedPositions(source: string, position: Position) {
		...
		for (const generatedPosition of generatedPositions) {
			if (isValidSourceMapPosition(generatedPosition)) {
				result.push(
					PositionConverters.toVSCodePosition(generatedPosition),
				);
			}
		}
		return result;
...
const sourceMapDataUrlRegExp =
	/^application\/json(?:;charset=[uU][tT][fF]-8)?;base64,(?<data>.*)/;

export function getInlineSourceMapData(
...
function isValidSourceMapLineRange(pos: source_map.LineRange) {
	return (
		isValidSourceMapPosition(pos) &&
		typeof pos.lastColumn === "number" &&
		pos.lastColumn >= pos.column
	);
}
...
function isValidSourceMapFindPosition(pos: source_map.FindPosition) {
	return isValidSourceMapPosition(pos) && isValidSourceMapBias(pos.bias);
}

function isValidSourceMapSourceFindPosition(
	pos: source_map.SourceFindPosition,
) {
	return isValidSourceMapFindPosition(pos) && typeof pos.source === "string";
}
...
function isValidMappingItemWithoutSource(
	item: Partial<ValidMappingItemWithoutSource | ValidMappingItemWithSource>,
): item is ValidMappingItemWithoutSource {
	return (
		item !== null &&
		item !== undefined &&
		typeof item.generatedLine === "number" &&
		item.generatedLine >= 0 &&
		typeof item.generatedColumn === "number" &&
		item.generatedColumn >= 0 &&
...
function isValidMappingItemWithSource(
	item: Partial<ValidMappingItemWithoutSource | ValidMappingItemWithSource>,
): item is ValidMappingItemWithSource {
	return (
		item !== null &&
		item !== undefined &&
		typeof item.generatedLine === "number" &&
		...
		item.originalLine >= 0 &&
		typeof item.originalColumn === "number" &&
...
function isValidMappingItem(
	item: Partial<ValidMappingItemWithoutSource | ValidMappingItemWithSource>,
): item is ValidMappingItemWithoutSource | ValidMappingItemWithSource {
	return (
		isValidMappingItemWithoutSource(item) ||
...
namespace PositionConverters {
	...
	// > - `line`: [...] Line numbers in this library are 1-based (note that the underlying source map specification uses 0-based line numbers -- this library handles the translation)
	// > - `column`: [...] Column numbers in this library are 0-based.
	export function toSourceMapPosition(
		position: Position,
	): source_map.Position {
