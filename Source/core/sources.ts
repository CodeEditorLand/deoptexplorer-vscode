...
import { type MessageItem, Uri, window, workspace } from "vscode";
import {
	getCanonicalUri,
	walkUpContainingDirectories,
} from "#extension/services/canonicalPaths.js";
...
import {
	...
	getInlineSourceMapData,
} from "./sourceMap";
import {
	ensureUriTrailingDirectorySeparator,
	relativeUriFragment,
	resolveUri,
	uriExtname,
} from "./uri";
...
export type UserFileResolution = SkipFileResolution | RedirectFileResolution;

export type UserFileResolver = (
	file: Uri,
) => UserFileResolution | Promise<UserFileResolution>;
...
const localFileOk: OkLocalFileResolution = Object.freeze({
	result: "ok",
	local: true,
});
const nonlocalFileOk: OkNonlocalFileResolution = Object.freeze({
	result: "ok",
	local: false,
});
const fileSkip: SkipFileResolution = Object.freeze({ result: "skip" as const });
const fileMissing: MissingFileResolution = Object.freeze({
...
export class Sources {
	...
	private _getExistingContentWorker(resolved: Uri) {
		return (
			this._scriptUrlToScript.get(resolved)?.text ??
	...
	private _getExistingResolutionWorker(
		file: Uri,
	): InternalFileResolution | undefined {
	...
	async resolveAsync(
		file: Uri,
		onMissing?: UserFileResolver | "prompt",
	): Promise<FileResolution> {
		return (
			this._tryResolveUsingExistingResolution(file, onMissing) ||
	...
	private _tryResolveUsingExistingResolution(
		file: Uri,
		onMissing: UserFileResolver | "prompt" | undefined,
	...
	private async _tryResolveUsingFileSystem(
		file: Uri,
	): Promise<FileResolution | undefined> {
	...
	private async _tryResolveUsingDirectoryRedirection(
		file: Uri,
	): Promise<FileResolution | undefined> {
		if (!isFileSystemLocation(file)) return undefined;
		for (const dirname of walkUpContainingDirectories(file)) {
			...
			if (directory) {
				const candidate = getCanonicalUri(
					resolveUri(
						ensureUriTrailingDirectorySeparator(directory.dirname),
						relativeUriFragment(dirname, file),
					),
				);
				const content = await tryReadFileAsync(candidate);
				if (content !== undefined) {
					return this._recordRedirectResolution(
						file,
						candidate,
						content,
						/*local*/ true,
	...
	private async _tryResolveUsingUiResolution(
		file: Uri,
		onMissing: UserFileResolver | "prompt" | undefined,
	): Promise<FileResolution | undefined> {
		if (onMissing === "prompt") {
			...
			return this._recordRedirectResolution(
				file,
				redirect,
				await readFileAsync(redirect),
				/*local*/ true,
			);
		}
	}

	private async _tryResolveUsingCustomResolution(
		file: Uri,
		onMissing: UserFileResolver | "prompt" | undefined,
	): Promise<FileResolution | undefined> {
		if (typeof onMissing === "function") {
			...
			return resolution.result === "skip"
				...
				: this._recordRedirectResolution(
						file,
						resolution.file,
						await readFileAsync(resolution.file),
						/*local*/ false,
					);
		}
	}

	private _getFileResolution(
		resolution: InternalFileResolution,
	): FileResolution {
		return isSkipLike(resolution)
			? fileSkip
			: isOkLike(resolution)
				? resolution.local
					? localFileOk
	...
	private _recordSkipResolution(
		file: Uri,
		resolution: SkipFileResolution | MissingFileResolution,
	) {
		this.delete(file);
		return this._recordResolution(file, resolution);
	}

	private _recordOkResolution(
		file: Uri,
		resolution: OkFileResolution | RedirectTargetFileResolution,
		content: string,
	) {
		...
		if (
			...
			isOkLike(existing) &&
			isOkLike(resolution) &&
			!(existing.result === "ok" && resolution.result === "ok")
		) {
			const merged: RedirectTargetFileResolution = {
				result: "target",
	...
	private _recordRedirectResolution(
		file: Uri,
		redirect: Uri,
		content: string,
		local: boolean,
		...
		if (redirect === file) {
			return this._recordOkResolution(
				file,
				local ? localFileOk : nonlocalFileOk,
				content,
			);
		} else {
			this._recordOkResolution(
				redirect,
				{
					result: "target",
					from: new StringSet(uriToString).add(file),
					local,
				},
				content,
			);
			...
			if (!directory)
				this._directoryRedirections.set(
					fileDirname,
					(directory = {
						dirname: redirectDirname,
	...
	delete(file: Uri) {
		...
		if (resolution !== undefined) {
			...
			if (resolution.result === "redirect") {
				...
				if (
					directory?.originalFiles.delete(file) &&
					directory.originalFiles.size === 0
				) {
				...
				if (target?.result === "target" && target.from.delete(file)) {
					if (
						target.from.size === 1 &&
						target.from.has(resolution.file)
					) {
	...
	getExistingSourceMap(file: Uri): SourceMap | "no-sourcemap" | undefined {
		...
		if (resolved !== undefined) {
			...
			return sourceMap === "no-sourcemap" ||
				sourceMap instanceof SourceMap
				? sourceMap
				: undefined;
		}
	}
	 ...
	 */
	private _getExistingSourceMapWorker(
		resolved: Uri,
	): SourceMap | Uri | "no-sourcemap" | undefined {
	...
	async getSourceMapAsync(
		file: Uri,
		onMissing?: UserFileResolver | "prompt",
	): Promise<SourceMap | "no-sourcemap"> {
		let resolved = this._resolveFile(file);
		if (resolved === undefined) {
			if (!isFileSystemLocation(file)) {
				return "no-sourcemap";
		...
		if (sourceMap instanceof Uri) {
			const data = isFileSystemLocation(sourceMap)
				? await tryReadFileAsync(sourceMap)
				: undefined;
			if (data !== undefined) {
				try {
...
function isOkLike(
	resolution: InternalFileResolution,
): resolution is OkFileResolution | RedirectTargetFileResolution {
	return resolution.result === "ok" || resolution.result === "target";
}

function isSkipLike(
	resolution: InternalFileResolution,
): resolution is SkipFileResolution | MissingFileResolution {
	return resolution.result === "skip" || resolution.result === "missing";
}

function mergeResolution(
	target: RedirectTargetFileResolution,
	file: Uri,
	other: OkFileResolution | RedirectTargetFileResolution,
) {
	if (other.result === "ok") {
