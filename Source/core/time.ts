...
function getNanoseconds(value: string | number | bigint, scale: Scale) {
	return typeof value === "bigint"
		? value * getNanosecondScale(scale)
		: typeof value === "number"
		? getNanosecondsFromFloat(value, scale)
		: typeof value === "string"
		? getNanosecondsFromDecimal(new Decimal(value), scale)
		: assertNever(value);
}
...
export class TimeTicks {
	...
	equals(other: TimeTicks) {
		return Equaler.defaultEqualer.equals(
			this._nanoseconds,
			other._nanoseconds
		);
	}
	...
	compareTo(other: TimeTicks) {
		return Comparer.defaultComparer.compare(
			this._nanoseconds,
			other._nanoseconds
		);
	}
...
export const TimeTicksComparer = Comparer.create<TimeTicks>((x, y) =>
	x.compareTo(y)
);
...
export class TimeDelta {
	...
	inNanosecondsF() {
		if (
			this._delta < BigInt(Number.MIN_SAFE_INTEGER) ||
...
export const TimeDeltaComparer = Comparer.create<TimeDelta>((x, y) =>
	x.compareTo(y)
);
