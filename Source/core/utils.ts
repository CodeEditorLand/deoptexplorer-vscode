...
export function binarySearchKey<T, K>(
	array: readonly T[],
	key: K,
...
export function getNullableEqualer<T>(
	equaler: Equaler<T>
	...
	if (!nullableEqualer)
		weakNullableEqualers.set(
			equaler,
			(nullableEqualer = Equaler.create(
...
export function equateNullable<T>(
	a: T | null | undefined,
	b: T | null | undefined,
	equaler?: Equaler<T>
): boolean {
	return (
		...
		(a !== undefined &&
			...
			b !== null &&
			b !== undefined &&
			(equaler ?? Equaler.defaultEqualer).equals(a, b))
	);
}

export function hashNullable<T>(
	a: T | null | undefined,
	equaler?: Equaler<T>
): number {
	return a === undefined || a === null
		? Equaler.defaultEqualer.hash(a)
		: (equaler || Equaler.defaultEqualer).hash(a);
}

const weakNullableComparers = new WeakMap<Comparer<any>, Comparer<any>>();

export function getNullableComparer<T>(
	comparer: Comparer<T>
	...
	if (!nullableComparer)
		weakNullableComparers.set(
			comparer,
			(nullableComparer = Comparer.create((left, right) =>
...
export function compareNullable<T>(
	...
	return (a ??= null) === (b ??= null)
		...
		: b === null
		? +1
		: (comparer || Comparer.defaultComparer).compare(a, b);
}
