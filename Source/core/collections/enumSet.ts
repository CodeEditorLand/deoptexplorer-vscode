...
export class ImmutableEnumSet<T extends string | number> {
	 ...
	 */
	static equals<T extends string | number>(
		left: ImmutableEnumSet<T>,
		right: ImmutableEnumSet<T>,
	) {
		if (left === right) {
			return true;
		}
	...
	forEach(
		cb: (value: T, key: T, set: ImmutableEnumSet<T>) => unknown,
		thisArg?: unknown,
	): void {
		for (const value of this._getOrderedSet()) {
	...
	static {
		ImmutableEnumSet.prototype[Symbol.iterator] =
			ImmutableEnumSet.prototype.values;
	}
