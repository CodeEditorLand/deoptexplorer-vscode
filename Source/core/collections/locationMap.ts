...
export class LocationMap<T> {
	...
	forEach(
		cb: (value: T, key: Location, map: LocationMap<T>) => unknown,
		thisArg?: unknown,
	) {
	...
	*keys() {
		for (const ranges of this._files.values())
			for (const [key] of ranges.values()) yield key;
	}

	*values(): Generator<T, void> {
		for (const ranges of this._files.values())
			for (const [, value] of ranges.values()) yield value;
	}

	*entries(): Generator<[Location, T], void> {
		for (const ranges of this._files.values())
			for (const [key, value] of ranges.values()) yield [key, value];
	}
