...
export class RangeMap<T> {
	...
	private _ranges: SplayTree<
		number,
		SplayTree<
			number,
			[Range, T] | SplayTree<number, SplayTree<number, [Range, T]>>
		>
	> = new SplayTree();
	private _size = 0;
	...
	has(key: Range) {
		...
		const entry = this._ranges
			.find(startLine)
			?.value.find(startCharacter)?.value;
	...
	findAll(positionOrRange: Position | Range) {
		...
		for (
			let startLine = this._ranges.findGreatestLessThan(start.line);
			startLine;
			startLine = startLine.left
		) {
			for (
				let startCharacter =
					startLine.key === start.line
						? startLine.value.findGreatestLessThan(start.character)
						: startLine.value.findMax();
				startCharacter;
				startCharacter = startCharacter.left
			) {
				if (Array.isArray(startCharacter.value)) {
					if (startCharacter.value[0].contains(positionOrRange)) {
						entries.unshift([
							startCharacter.value[0],
							startCharacter.value[1],
						]);
					}
				} else {
					const inOrderEntries: [Range, T][] = [];
					for (
						let endLine = startCharacter.value.findLeastGreaterThan(
							end.line,
						);
						endLine;
						endLine = endLine.right
					) {
						for (
							let endCharacter =
								endLine.key === end.line
									? endLine.value.findLeastGreaterThan(
											end.character,
										)
									: endLine.value.findMin();
							endCharacter;
							endCharacter = endCharacter.right
						) {
							if (
								endCharacter.value[0].contains(positionOrRange)
							) {
								inOrderEntries.push([
									endCharacter.value[0],
	...
	set(key: Range, value: T) {
		...
		if (!startCharacters)
			this._ranges.insert(startLine, (startCharacters = new SplayTree()));
		...
		if (!endLines) {
			...
			if (!existingEndCharacters)
				endLines.insert(
					existingEndLine,
					(existingEndCharacters = new SplayTree()),
				);
		...
		if (!endCharacters)
			endLines.insert(endLine, (endCharacters = new SplayTree()));

		const entry = endCharacters.find(endCharacter)?.value;
		if (entry) {
			entry[1] = value;
		} else {
			this._size++;
