...
describe("locationMap", () => {
	...
	const implementations: {
		...
		{
			name: "Map ×5",
			LocationMap: LocationMapUsingMaps,
			locationMap: undefined!,
		},
		{
			name: "SplayTree ×5",
			LocationMap: LocationMapUsingSplayTreeRangeMap,
			locationMap: undefined!,
		},
		{
			name: "Map->SplayTree ×4",
			LocationMap: LocationMapUsingMapRangeMap,
			locationMap: undefined!,
		},
		{
			name: "Map->SplayTreeWithComparer",
			LocationMap: LocationMapUsingMapRangeMapUsingSplayTreeWithComparer,
			locationMap: undefined!,
		},
	];
				...
				...generate(
					...
					(i) =>
						[
							randomLocation({
								uris,
								collapseChance: collapsedRangePopulation,
							}),
							i,
						] as const,
				),
		...
		beforeAll(() => {
			...
			keys = Array.from({ length: keyCount }).map(
				() =>
					...
					randomLocation({
						uris: randomBoolean(missPopulation) ? undefined : uris,
					}),
			);
		});

		benchmark.each(implementations)(
			"$name",
			({ locationMap }) => {
