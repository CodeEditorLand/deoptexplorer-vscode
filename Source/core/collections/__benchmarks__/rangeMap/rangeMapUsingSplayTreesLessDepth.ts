    ...
    * findAllIntersecting(positionOrRange: Position | Range): Generator<[Range, T], void> {
    ...
    set(key: Range, value: T) {
        ...
        if (!endLines) {
            ...
            startCharacters.insert(start.character, endLines = new SplayTree());
        }
            ...
            const existingEndCharacters = ensureLine(endLines, existingEntry[0].end);
...
function isCollapsed<T>(entry: [Range, T] | EndLineTree<T> | undefined): entry is [Range, T] {
...
function characterIsBefore<T>(line: LineNode<T>, character: CharacterNode<T>, position: Position) {
    return line.key < position.line || line.key === position.line && character.key < position.character;
}
