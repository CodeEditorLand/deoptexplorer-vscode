...
type StartLineTree<T> = LineTree<[Range, T] | EndLineTree<T>>;
...
export class RangeMap<T> {
	...
	*findAllContaining(
		...
		startLoop: for (const startLine of iterateAscending(
			least(this._ranges),
		)) {
			...
			for (const startCharacter of iterateAscending(
				...
				if (isCollapsed(startCharacter.value)) {
					...
					for (const endLine of iterateAscending(
						leastLineGreaterThan(startCharacter.value, end),
					)) {
	...
	findNearestContaining(
		...
		// Since all entries must contain the range, all entry starts must occur on or before the range start
		for (const startLine of iterateDescending(
			greatestLineLessThan(this._ranges, start),
		)) {
			for (const startCharacter of iterateDescending(
				greatestCharacterLessThan(startLine, start),
			)) {
				if (isCollapsed(startCharacter.value)) {
					const [key, value] = startCharacter.value;
	...
	*findAllContainedBy(range: Range): Generator<[Range, T], void> {
		...
		for (const startLine of iterateAscending(
			leastLineGreaterThan(this._ranges, start),
		)) {
	...
	*keys(): Generator<Range, void> {
		for (const { value: startCharacters } of iterateAscending(
			least(this._ranges),
		)) {
			for (const { value: endLines } of iterateAscending(
				least(startCharacters),
			)) {
				if (isCollapsed(endLines)) {
					const [key] = endLines;
					...
					for (const { value: endCharacters } of iterateAscending(
						...
						for (const {
							value: [key],
						} of iterateAscending(least(endCharacters))) {
							yield key;
						}
					}
				}
...
function isCollapsed<T>(
	entry: [Range, T] | EndLineTree<T> | undefined,
): entry is [Range, T] {
...
function lineIsBefore<T>(line: LineNode<T>, position: Position) {
	return line.key < position.line;
}

function characterIsAfter<T>(
	line: LineNode<T>,
	character: CharacterNode<T>,
	position: Position,
) {
	return (
		line.key > position.line ||
		(line.key === position.line && character.key > position.character)
	);
}

function characterIsBefore<T>(
	line: LineNode<T>,
	character: CharacterNode<T>,
	position: Position,
) {
...
function* iterateDescending<T>(
	node: SplayTree.Node<number, T> | null,
