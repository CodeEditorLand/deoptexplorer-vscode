...
import { type Address, parseAddress, toAddress } from "#core/address.js";
...
import {
	type FuncInfo,
	WindowsCppEntriesProvider as V8WindowsCppEntriesProvider,
...
export class WindowsCppEntriesProvider extends V8WindowsCppEntriesProvider {
	...
	//                                                          ^^^^^^^^
	//                                                          <address>
	static readonly IMAGE_BASE_RE =
		/^0000:00000000\s+___ImageBase\s+(?<address>[0-9a-fA-F]{8}[0-9a-fA-F]{8}?).*$/;
	...
	//                 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^  ^^^^^^^^
	//                 <name>                                   <address>
	static readonly FUNC_RE =
		/^0001:[0-9a-fA-F]{8}\s+(?<name>[_?@$0-9a-zA-Z]+)\s+(?<address>[0-9a-fA-F]{8}[0-9a-fA-F]{8}?).*$/;
	...
	//               ^^^^^^^^                      ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	//               <rva>                         <name>
	static readonly DUMPBIN_MAP_FUNC_RE =
		/^(?<section>\d{4}):(?<address>[0-9a-fA-F]{8})\s+(?<rva>[0-9a-fA-F]{8})\s+[0-9a-fA-F]{8}\s+(?<size>[0-9]+)\s+(?<name>[_?@$0-9a-zA-Z]+)/;
	...
	//  `\?\$`          - suffix (optional)
	static readonly SYMBOL_PREFIX_RE =
		/^\?(?:\?_?[0-9A-Z]|_)?(?:\?\$)?(?=[A-Za-z])/;
		...
		if (this.useDbghelp !== true) {
			...
			try {
				...
				if ((e as NodeJS.ErrnoException).code !== "ENOENT") {
					output.warn(
						`Unable to load symbols from '${mapFileName}':`,
						e,
					);
				}
			}
					...
					try {
						...
						if (hasPdb) {
							try {
								...
								output.warn(
									`Unable to dump symbols from '${pdbFileName}':`,
									e,
								);
							}
						}
						...
						try {
							...
							output.warn(
								`Unable to clean up temp file '${tempFile}':`,
								e,
							);
						}
					}
	...
	private createTempFile(base: string) {
		...
		for (let i = 0; i < 1_000; i++) {
			const tempFile = path.join(
				tmpdir,
				`${base}~${randomInt(0x7fffffff)
					.toString(16)
					.padStart(8, "0")}.dumpbin`,
			);
			try {
	...
	private findDumpbinPath() {
		...
		if (programData) {
			...
			if (instanceNames) {
				...
				instances.sort(
					(a, b) =>
						-a.installationVersion.compare(b.installationVersion),
				);
				...
				for (const { installationPath } of instances) {
					...
					if (versionNames) {
						...
						msvcToolsVersions.sort(
							(a, b) => -a.version.compare(b.version),
						);

						// discover a recent version of dumpbin.exe
						for (const { name } of msvcToolsVersions) {
							const dumpbinPath = path.join(
								msvcDir,
								name,
	...
	private isValidImageBase(imageBase: bigint) {
		switch (this.moduleType) {
			case ".exe":
				if (
					imageBase === WindowsCppEntriesProvider.EXE_IMAGE_BASE_32 ||
	...
	protected parseNextLine(): FuncInfo | null | false {
		...
		const regexp =
			this.symbolsFormat === "dumpbin-map"
				? WindowsCppEntriesProvider.DUMPBIN_MAP_FUNC_RE
