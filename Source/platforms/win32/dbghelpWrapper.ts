...
import type { FuncInfo } from "#v8/tools/cppEntriesProvider.js";
import * as dbghelp from "./api/dbghelp";
import * as kernel32 from "./api/kernel32";
import { type PSTR, sizeof } from "./api/win32";
		...
		for (const { using, fail } of Disposable.scope())
			try {
				...
				if (fInitialized) {
					...
					deferrals.use(() => {
						if (!this.#dbghelp.SymCleanup(hProcess))
				...
				const prevOptions = this.#dbghelp.SymGetOptions();
				...
				if (!fInitialized) {
					...
					deferrals.use(() => {
						this.#dbghelp.SymSetOptions(prevOptions);
					});
				}
				...
				if (progress) {
					if (
						!this.#dbghelp.SymRegisterCallback64(
							hProcess,
							(
								_hProcess,
								ActionCode,
				...
				if (base === kNullAddress) {
					...
					deferrals.use(() => {
						if (!this.#dbghelp.SymUnloadModule64(hProcess, base))
							throw new kernel32.Win32Error();
					});
				}
