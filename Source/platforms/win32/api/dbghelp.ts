...
function dbghelpFactory() {
	...
	const dbghelpFile =
		process.arch === "x64"
			? Uri.joinPath(extensionUri, "bin/x64/dbghelp.dll").fsPath
			: process.arch === "x86"
				? Uri.joinPath(extensionUri, "bin/x86/dbghelp.dll").fsPath
				: undefined;
...
export class Dbghelp {
	...
	SymCleanup(handle: HANDLE): BOOL {
		return this.#dbghelp.SymCleanup(handle);
	}
	...
	SymSetOptions(opts: DWORD): DWORD {
		return this.#dbghelp.SymSetOptions(opts);
	}
	...
	SymGetModuleInfo64(
		...
		return this.#dbghelp.SymGetModuleInfo64(hProcess, qwAddr, ModuleInfo);
	}

	SymUnloadModule64(hProcess: HANDLE, BaseOfDll: DWORD64) {
		return this.#dbghelp.SymUnloadModule64(hProcess, BaseOfDll);
	}
	...
	SymSetSearchPath(hProcess: HANDLE, SearchPath: PCSTR): BOOL {
		return this.#dbghelp.SymSetSearchPath(hProcess, SearchPath);
	}

	SymSetSearchPathW(hProcess: HANDLE, SearchPath: PCWSTR): BOOL {
		return this.#dbghelp.SymSetSearchPathW(hProcess, SearchPath);
	}
	...
	SymNext(hProcess: HANDLE, si: PSYMBOL_INFO): BOOL {
		return this.#dbghelp.SymNext(hProcess, si);
	}

	SymNextW(hProcess: HANDLE, si: PSYMBOL_INFOW): BOOL {
		return this.#dbghelp.SymNextW(hProcess, si);
	}
