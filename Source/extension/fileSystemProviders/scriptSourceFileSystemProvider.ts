...
export function getScriptSourceLocation(
	location: Location,
	...
	return uri === location.uri
		? location
		: uri
		? new Location(uri, location.range)
		: undefined;
}
			...
			switch (source.uri.scheme) {
				...
				default:
					path = `/${encodeURIComponent(
						source.uri.toString(/*skipEncoding*/ true)
					)}`;
					break;
			}
		}
	...
	watch(
		...
		stack.use(
			events.onDidCloseLogFile(() =>
				this.didChangeFile.fire([{ type: FileChangeType.Deleted, uri }])
			)
		);
		stack.use(
	...
	stat(uri: Uri): FileStat | Thenable<FileStat> {
		...
		const exists =
			source.scriptId && source.scriptId > 0
				...
				: source.uri
				? openedLog?.sources.has(source.uri)
				: false;
	...
	readFile(uri: Uri): Uint8Array | Thenable<Uint8Array> {
		...
		const text =
			source.scriptId && source.scriptId > 0
				...
				: source.uri
				? openedLog?.sources.getExistingContent(source.uri)
				: undefined;
