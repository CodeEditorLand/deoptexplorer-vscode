	...
	constructor(
		...
		const dispatchTableProxy = new Proxy<DispatchTable>(
			Object.create(null),
			{
				...
				has: (_target, p) =>
					typeof p === "string" &&
					this._getDispatcher(p) !== undefined,
				get: (_target, p, _receiver) =>
					typeof p === "string" ? this._getDispatcher(p) : undefined,
			...
			if (this._version) {
				for (const [key, versionRange] of this._versionRanges) {
					if (versionRange.test(this._version.semver)) {
						matchingVersionDispatchTables.push(
							this._versionedDispatchTable[key],
						);
					}
				}
			}
