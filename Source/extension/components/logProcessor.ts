...
import { type IcState, parseIcState } from "#v8/enums/icState.js";
import { IcType } from "#v8/enums/icType.js";
import { MapEvent, parseMapEvent } from "#v8/enums/mapEvent.js";
import { type VmState, parseVmState } from "#v8/enums/vmState.js";
import { TickSample } from "#v8/tickSample.js";
import {
	type CodeEntry,
	type DynamicCodeEntry,
	SharedFunctionCodeEntry,
} from "#v8/tools/codeentry.js";
import { CodeMap } from "#v8/tools/codemap.js";
import { ConsArray } from "#v8/tools/consarray.js";
import {
	type CppEntriesProvider,
	getCppEntriesProvider,
} from "#v8/tools/cppEntriesProvider.js";
import {
	type Parser,
	type Parsers,
	cancelTokenArg,
...
const mapDetailsPropertyLikeRegExp = /^ {2}\[\d+\]:/;
const mapDetailsPropertyRegExp =
	/^ {2}\[\d+\]: (?:0x)?[a-fA-F0-9]+:? <(?<type>String\[#?\d+\]|Symbol): (?<key>[^>]*)> \((?:const )?(?:data|accessor)(?: field(?: \d+)?(?::(?<mnemonic>\w+))?| descriptor)(?:, p: \d+)?(?:, attrs: \[(?<attrs>[W_][E_][C_])\])?\) @ (?:Any|None|Class\((?<classMapAddress>[a-fA-F0-9]+)\))?/;
const mapDetailsPropertyRegExp2 =
	/^ {2}\[\d+\]: (?:0x)?[a-fA-F0-9]+:? \[(?<type>[^\]]+)\] in (?<space>\w+): u?#(?<key>(?:(?! \((?:const )?(?:data|accessor)).)*) \((?:const )?(?:data|accessor)(?: field(?: \d+)?(?::(?<mnemonic>\w+))?| descriptor)(?:, p: \d+)?(?:, attrs: \[(?<attrs>[W_][E_][C_])\])?\) @ (?:Any|None|Class\((?<classMapAddress>[a-fA-F0-9]+)\))?/;
...
export class LogProcessor {
	...
	constructor({
		...
		const parseTimeTicks = (text: string) => {
			const timestamp = TimeTicks.sinceOrigin(
				TimeDelta.fromMicroseconds(text),
			);
			if (timestamp.compareTo(this._lastTimestamp) >= 0)
				this._lastTimestamp = timestamp;
			return timestamp;
		};

		const parseCodeKindForVersion = (text: string): CodeKind =>
			parseCodeKind(text, this._version);
		const parseVmStateForVersion = (text: string): VmState =>
			parseVmState(text, this._version);
		...
		this._reader = new VersionedLogReader(
			{
				...
				"*": {
					...
					"code-creation": DISPATCHER({
						parsers: [
							parseString,
							parseCodeKindForVersion,
							parseTimeTicks,
							parseAddress,
							parseInt32,
							parseString,
							{ parser: parseAddress, optional: true },
							{ parser: parseFunctionState, optional: true },
						],
					...
					"code-disable-optimization": {
						...
						processor:
							this.processCodeDisableOptimization.bind(this),
					},
					...
					LoadGlobalIC: DISPATCHER({
						...
						processor: this.processIcEventUsingLastTimestamp.bind(
							this,
							IcType.LoadGlobalIC,
						),
					}),
					...
					profiler: DISPATCHER({
						parsers: [
							parseString,
							{ parser: parseInt32, optional: true },
						],
					...
					new: DISPATCHER({
						parsers: [parseString, parseAddress, Number.parseInt],
	...
	async process(
		...
		try {
			await measureAsync("processContent", () =>
				this.processContent(content, progress, token, estimatedLogSize),
			);
			...
			await measureAsync("resolveFiles", async () => {
				...
				for (const file of workQueue.values()) {
					...
					// Resolve the file
					const resolution =
						this._sources.getExistingResolution(file) ??
						(await this._sources.resolveAsync(file));
					if (resolution.result === "skip") continue;

					// Resolve the source map
					const sourceMap =
						this._sources.getExistingSourceMap(file) ??
						(await this._sources.getSourceMapAsync(file));
			...
			measureSync("resolveLocations", () => {
				...
				const workItems: Iterable<ResolutionWorkItem> =
					from<ResolutionWorkItem>(empty())
						...
						.concat(
							flatMap(
								flatMap(this._maps.exportValues(), identity),
								(map) => map.updates,
							),
						);
				...
				for (const entry of workItems) {
					...
					if (resolution.result === "redirect") {
						switch (entry.kind) {
							case "function":
								this._functions.delete(entry.filePosition);
								break;
							case "ic":
								this._ics.delete(entry.filePosition);
								break;
							case "deopt":
								this._deopts.delete(entry.filePosition);
								break;
						}
						...
						switch (entry.kind) {
							case "function":
								this._functions.set(entry.filePosition, entry);
								break;
							case "ic":
								this._ics.set(entry.filePosition, entry);
								break;
							case "deopt":
								this._deopts.set(entry.filePosition, entry);
								break;
						}
					}
					...
					if (resolution.result !== "skip") {
						...
						if (sourceMap !== "no-sourcemap") {
							let sourceLocation: Location | undefined =
								sourceMap.toSourceLocation(
									entry.filePosition.range.start,
								);
							if (sourceLocation) {
								const canonicalUri = getCanonicalUri(
									sourceLocation.uri,
								);
								const resolution =
					...
					if (entry.generatedFilePosition) {
						if (
							this._sources.has(entry.generatedFilePosition.uri)
						) {
							this._generatedPaths.add(
								entry.generatedFilePosition.uri,
							);
							switch (entry.kind) {
								case "function":
									this.getFileEntry(
			...
			return new LogFile(
				...
				new StringMap<MapId, MapEntry>(
					...
					flatMap(
						this._maps.exportKeysAndValues(),
						([address, maps]) =>
							map(
								maps,
								(map, index) =>
									[new MapId(address, index), map] as [
										MapId,
										MapEntry,
									],
							),
	...
	private addEntry(entry: Entry) {
		...
		if (entry.generatedFilePosition) {
			...
			if (!entries)
				this._entries.set(entry.generatedFilePosition, (entries = {}));
			(entries[entry.kind] as Entry) = entry;
		}
	}
	...
	private getFileEntry(file: Uri) {
		...
		if (!fileEntry)
			this._files.set(
				file,
				(fileEntry = { functions: [], ics: [], deopts: [] }),
			);
		return fileEntry;
	}
	...
	// event source: https://github.com/v8/v8/blob/01c670e416310453b01533a85057a3e2db3ac64f/src/logging/log.cc#L1087
	private async processSharedLibrary(
		libName: string,
		...
		const entry = this._profile.addLibrary(
			libName,
			startAddress,
			endAddress,
		);
	...
	private processCodeCreation(
		...
		if (funcStartAddress !== undefined && state !== undefined) {
			const code = this._profile.addFuncCode(
				type,
				name,
				timestamp,
				startAddress,
			...
			const code = this._profile.addCode(
				type,
				name,
				timestamp,
				startAddress,
	...
	// event definition: https://github.com/v8/v8/blob/01c670e416310453b01533a85057a3e2db3ac64f/src/logging/code-events.h#L38
	// event source: https://github.com/v8/v8/blob/01c670e416310453b01533a85057a3e2db3ac64f/src/logging/log.cc#L1567
	private processSharedFunctionMove(
		fromAddress: Address,
		toAddress: Address,
	) {
	...
	private processScriptSource(scriptId: number, url: string, source: string) {
		const uri = url
			? getCanonicalUri(
					isPathOrUriString(url)
						? pathOrUriStringToUri(url)
						: resolveUri(
	...
	private processCodeSourceInfo(
		...
		return this._profile.addSourcePositions(
			startAddress,
			scriptId,
			startPos,
			endPos,
	...
	private processIcEventUsingLastTimestamp(
		type: IcType,
		pc: Address,
		line: number,
		column: number,
	...
	private processIcEvent(
		...
		if (code) {
			...
			const codeFilePosition =
				code.filePosition ?? code.functionName.filePosition;
			if (codeFilePosition) {
				const uri = codeFilePosition.uri;
				const icFilePosition = new Location(
					uri,
					new Position(
						Math.max(0, line - 1),
						Math.max(0, column - 1),
				...
				if (map) {
					...
					if (!mapReferences)
						this._mapReferences.set(
							map,
							(mapReferences = new Set()),
						);
	...
	// event source: https://github.com/v8/v8/blob/01c670e416310453b01533a85057a3e2db3ac64f/src/logging/log.cc#L1484
	private processCodeDeopt(
		timestamp: TimeTicks,
		size: number,
		pc: Address,
		inliningId: number,
		scriptOffset: number,
		bailoutType: DeoptimizeKind,
		deoptPosition: DeoptPosition,
		deoptReason: string,
	) {
		const code = this.findProfileEntry(pc);
		assert(code);
		...
		if (update.functionEntry) {
			update.functionEntry?.timeline.push({
				event: "deopt",
				timestamp,
				entry,
				update,
	...
	private processMapDetails(
		...
		if (constructorMatch) {
			const [, constructorAddressText, constructorName, sfiAddressText] =
				constructorMatch;
			const sfi = sfiAddressText
				? tryParseAddress(sfiAddressText)
				: undefined;
			if (isAddress(sfi)) {
			...
			if (!map.constructorEntry) {
				const codeAddress = constructorAddressText
					? tryParseAddress(constructorAddressText)
					: undefined;
				if (isAddress(codeAddress)) {
					map.constructorEntry ??=
						this.findFunctionByCodeAddress(codeAddress);
				}
			}
		...
		map.instanceSize = instanceSize
			? Number.parseInt(instanceSize, 10)
			: undefined;
		...
		for (const line of detailLines) {
			...
			// parse properties
			const match =
				mapDetailsPropertyRegExp.exec(line) ??
				mapDetailsPropertyRegExp2.exec(line);
			if (!match?.groups) {
				warn(
					"'map-details' line does not match the expected format:",
					line,
				);
				continue;
			}

			const { key, type, mnemonic, attrs, classMapAddress } =
				match.groups;
			const name = type === "Symbol" ? new SymbolName(key) : key;
			let property = map.properties.find((prop) =>
				PropertyNameEqualer.equals(prop.name, name),
			);
			if (!property)
				map.properties.push((property = new MapProperty(name)));
			if (attrs) {
			...
			switch (mnemonic) {
				...
				default:
					output.warn(
						`Unrecognized mnemonic '${mnemonic}' in map details: '${line}'`,
					);
					break;
			}
			if (isAddress(property.type)) {
				...
				if (targetMap) {
					targetMap.referencedBy.push(
						new MapReferencedByMapProperty(property),
					);
				}
			}
		}
	...
	private processMapEvent(
		...
		const fromMaps =
			fromAddress !== kNullAddress
				? this._maps.find(fromAddress)?.value
				: undefined;
		...
		if (!(fromAddress === kNullAddress || from)) {
			warn(
				`LogProcessor.processMapEvent(): Map not found for source map address ${formatAddress(
					fromAddress,
				)}`,
			);
		}
		if (!(toAddress === kNullAddress || to)) {
			warn(
				`LogProcessor.processMapEvent(): Map not found for target map address ${formatAddress(
					toAddress,
				)}`,
			);
		}
		...
		if (to) {
			if (to.updates.length === 0) {
				to.baseMap =
					from &&
					new MapReference(fromAddress, fromIndex, from, reason);
			}
		...
		if (from && to) {
			from.referencedBy.push(
				new MapReferencedByMap(
					new MapReference(toAddress, toIndex, to, reason),
				),
			);
			for (const sourceProperty of from.properties) {
				const existingProperty = to.properties.find((property) =>
					PropertyNameEqualer.equals(
						property.name,
						sourceProperty.name,
					),
				);
			...
			if (event === MapEvent.Transition) {
				...
				if (existingProperty) {
					existingProperty.map = new MapReference(
						toAddress,
						toIndex,
						to,
						reason,
					...
					to.properties.push(
						new MapProperty(name, {
							map: new MapReference(
								toAddress,
								toIndex,
								to,
								reason,
							),
							source: update.functionEntry,
							update,
						}),
					);
				}
	...
	// event definition: https://github.com/v8/v8/blob/01c670e416310453b01533a85057a3e2db3ac64f/src/logging/code-events.h#L40
	// event source: https://github.com/v8/v8/blob/01c670e416310453b01533a85057a3e2db3ac64f/src/logging/log.cc#L1741
	private processTickEvent(
		pc: Address,
		timestamp: TimeTicks,
		isExternalCallback: number,
		topOfStackOrExternalCallback: Address,
		vmState: VmState,
		stack: string[],
	) {
		// https://github.com/v8/v8/blob/6bbf2dfa5b3bde9a40527268741b739f7d7e4bd0/src/profiler/symbolizer.cc#L48
		const sample = new TickSample(
			pc,
			timestamp,
			!!isExternalCallback,
			topOfStackOrExternalCallback,
			vmState,
	...
	private changeMemoryCategorySize(
		...
		if (!memoryCategory) {
			...
			categories.set(
				name,
				(memoryCategory = new MemoryCategory(name, 0, 0)),
			);
		}
	...
	private processNewEvent(name: string, object: Address, size: number) {
		...
		if (existing) {
			log(
				`'new' event for category '${name}' at address ${formatAddress(
					object,
	...
	private processDeleteEvent(name: string, object: Address) {
		...
		if (!existing) {
			warn(
				`'delete' event for category '${name}' at ${formatAddress(
					object,
				)} referenced an allocation that was not recorded.`,
			);
			return;
		}
		...
		if (existing.name !== name) {
			warn(
				`'delete' event category '${name}' did not match existing memory category '${existing.name}'.`,
			);
		}
...
export type ProcessorParameter<T extends Parser> = T extends (
	s: string,
) => infer U
	? U
	: T extends { readonly parser: (s: string) => infer U; readonly rest: true }
		? U[]
		: T extends {
			...
			: T extends {
				...
				: T extends {
					...
					? string | undefined
					: T extends typeof commandNameArg
						? string
						: T extends typeof cancelTokenArg
							? CancellationToken | undefined
							: T extends typeof parseString
								? string
								: T extends typeof parseInt32
									? number
									: T extends typeof parseVarArgs
										? string[]
										: never;

type ProcessorParameters<A extends Parsers> = Extract<
	{ [P in keyof A]: ProcessorParameter<Extract<A[P], Parser>> },
	readonly any[]
>;
