...
import { Location, type Position, Range, SymbolKind, Uri } from "vscode";
...
function setReferenceLocation(
	locationKind: LocationKind,
	entry: Entry,
	uriOrLocation: Uri | Location | undefined,
	range?: Range,
) {
...
function setReferenceLocationFromNode(
	locationKind: LocationKind,
	entry: Entry,
...
function resolveLocations<T extends Entry>(
	locationKind: LocationKind,
	entry: T,
	tryResolveLocations: (
		locationKind: LocationKind,
...
function getRangeForNameOrKeywordOfFunction(
	document: TextDocumentLike,
...
function getRangeForKeywordOfConstructor(
	document: TextDocumentLike,
...
function getRangeForNameOfMethod(
	document: TextDocumentLike,
...
function getRangeForEqualsGreaterThanTokenOfArrowFunction(
	document: TextDocumentLike,
...
function setFunctionLocationsFromFunctionNode(
	locationKind: LocationKind,
	entry: FunctionEntry,
	document: TextDocumentLike,
	node: ts.FunctionDeclaration | ts.FunctionExpression,
...
function setFunctionLocationsForConstructorNode(
	locationKind: LocationKind,
	entry: FunctionEntry,
	document: TextDocumentLike,
	node: ts.ConstructorDeclaration,
...
function setFunctionLocationsForMethodNode(
	locationKind: LocationKind,
	entry: FunctionEntry,
	document: TextDocumentLike,
	node: ts.MethodDeclaration | ts.AccessorDeclaration,
...
function setFunctionLocationsForArrowFunctionNode(
	locationKind: LocationKind,
	entry: FunctionEntry,
	document: TextDocumentLike,
	node: ts.ArrowFunction,
...
function tryResolveFunctionLocationsFallback(
	locationKind: LocationKind,
	entry: FunctionEntry,
	filePosition: Location,
	document: TextDocumentLike,
	...
	if (token.kind === ts.SyntaxKind.OpenParenToken) {
		if (
			...
			// function () {}
			// ~~~~~~~~
			return setFunctionLocationsFromFunctionNode(
				locationKind,
				entry,
				document,
				token.parent,
		...
		if (ts.isConstructorDeclaration(token.parent)) {
			...
			// constructor() {}
			// ~~~~~~~~~~~
			return setFunctionLocationsForConstructorNode(
				locationKind,
				entry,
				document,
				token.parent,
		...
		if (
			...
			// get foo() {}
			//     ~~~
			return setFunctionLocationsForMethodNode(
				locationKind,
				entry,
				document,
				token.parent,
		...
		if (ts.isArrowFunction(token.parent)) {
			...
			// async () => {}
			//          ~~
			return setFunctionLocationsForArrowFunctionNode(
				locationKind,
				entry,
				document,
				token.parent,
		...
		if (
			...
			ts.isArrowFunction(token.parent.arguments[0])
		) {
			//  ▼
			// f(a => {})
			//     ~~
			return setFunctionLocationsForArrowFunctionNode(
				...
				entry,
				document,
				token.parent.arguments[0] as ts.ArrowFunction,
			);
		}
	}
	if (
		...
		token.parent.elements.length &&
		ts.isArrowFunction(token.parent.elements[0])
	) {
		// ▼
		// [a => {}]
		//    ~~
...
function tryResolveFunctionLocationsInJavaScript(
	locationKind: LocationKind,
	entry: FunctionEntry,
	filePosition: Location,
	document: TextDocumentLike,
...
function tryResolveFunctionLocationsInTypeScript(
	locationKind: LocationKind,
	entry: FunctionEntry,
	filePosition: Location,
	document: TextDocumentLike,
	...
	if (entry.generatedReferenceLocation && entry.generatedExtentLocation) {
		...
		if (sourceMap) {
			...
			if (
				...
				if (
					tryResolveFunctionLocationsFallback(
						locationKind,
						entry,
						filePosition,
						document,
...
function tryResolveFunctionLocations(
	locationKind: LocationKind,
	entry: FunctionEntry,
	filePosition: Location,
	document: TextDocumentLike,
	...
	if (isJavaScriptFile(filePosition.uri)) {
		if (
			tryResolveFunctionLocationsInJavaScript(
				locationKind,
				entry,
				filePosition,
				document,
...
export function resolveFunctionLocations(
	locationKind: LocationKind,
	entry: FunctionEntry,
) {
...
function tryResolveIcLocationInJavaScript(
	locationKind: LocationKind,
	entry: IcEntry,
	filePosition: Location,
	document: TextDocumentLike,
	...
	if (worst.type === IcType.LoadIC || worst.type === IcType.KeyedLoadIC) {
		...
		worst.type === IcType.LoadGlobalIC ||
		worst.type === IcType.StoreGlobalIC
	) {
...
function tryResolveIcLocationInTypeScript(
	locationKind: LocationKind,
	entry: IcEntry,
	filePosition: Location,
	document: TextDocumentLike,
		...
		if (sourceMap) {
			...
			if (location) {
				if (location.range.isEmpty) {
					if (
						tryResolveIcLocationInJavaScript(
							locationKind,
							entry,
							filePosition,
							document,
...
function tryResolveIcLocations(
	locationKind: LocationKind,
	entry: IcEntry,
	filePosition: Location,
	document: TextDocumentLike,
	...
	if (isJavaScriptFile(filePosition.uri)) {
		return tryResolveIcLocationInJavaScript(
			locationKind,
			entry,
			filePosition,
			document,
...
function tryResolveDeoptLocationInJavaScript(
	...
	if (
		...
		(worst?.bailoutType === DeoptimizeKind.Soft &&
...
function tryResolveDeoptLocationInTypeScript(
	locationKind: LocationKind,
	entry: DeoptEntry,
	filePosition: Location,
	document: TextDocumentLike,
	...
	if (entry.generatedReferenceLocation) {
		...
		if (sourceMap) {
			...
			if (location) {
				if (location.range.isEmpty) {
					if (
						tryResolveDeoptLocationInJavaScript(
							locationKind,
							entry,
							filePosition,
							document,
...
function tryResolveDeoptLocations(
	locationKind: LocationKind,
	entry: DeoptEntry,
	filePosition: Location,
	document: TextDocumentLike,
	...
	if (isJavaScriptFile(filePosition.uri)) {
		return tryResolveDeoptLocationInJavaScript(
			locationKind,
			entry,
			filePosition,
			document,
...
export function resolveDeoptLocations(
	locationKind: LocationKind,
	entry: DeoptEntry,
) {
