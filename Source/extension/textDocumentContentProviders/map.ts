...
import { TimeDelta, type TimeTicks } from "#core/time.js";
...
import {
	...
	getScriptSourceUri,
} from "../fileSystemProviders/scriptSourceFileSystemProvider";
import {
	type MapEntry,
	type MapEntryUpdate,
	MapId,
...
class MapDocumentContentProvider
	implements
		...
		DocumentLinkProvider
{
	...
	private getDocumentContentAndLinks(uri: Uri): MapDocument | undefined {
		...
		while (currentMapRef) {
			...
			for (let i = currentMapRef.map.updates.length - 1; i >= 0; i--) {
				...
				if (
					update.toMapId.address === currentMapRef.address &&
		...
		for (const prop of map.properties) {
			if (prop.source && prop.source !== lastSource) {
				const prefix = `// Added by ${
					prop.source.functionName ?? "(anonymous)"
				} (`;
				const link = formatLocation(prop.source.filePosition, {
					as: "file",
					include: "position",
				});
				const suffix = `):`;
			...
			markdown
				.table(
					[
						...
						{ text: "To", align: "left" },
						{ text: "Reason", align: "left" },
						{ text: "Location", align: "left" },
					],
					from(history).toArray((update) => {
						const reason = update.reason
							? update.reason
							: update.event === MapEvent.Transition &&
									update.name instanceof SymbolName
								? `Added property ${update.name}`
								: update.event === MapEvent.Transition &&
										update.name
									? `Added property '${update.name}'`
									: update.event === MapEvent.InitialMap
										? `Map created`
										: "";
						let location = "";
						if (update.filePosition) {
							location += `${
								update.functionName ?? "(anonymous)"
							} ${formatLocation(update.filePosition, {
								as: "file",
								include: "position",
							})}`;
						}
						return [
							`${formatTimeRelativeTo(
								update.timestamp,
								startTime,
							)}`,
							update.event,
							...
							reason,
							{
								...
								onWrite: (position) => {
									if (update.filePosition) {
										const uri = getScriptSourceUri(
											update.filePosition.uri,
											openedLog?.sources,
										);
										if (uri) {
											const line =
												writer.line + position.line;
											const start =
												position.character +
												(update.functionName?.length ??
													0) +
												1;
											const end =
												start +
											...
											const pos =
												update.filePosition.range.start;
											documentLinks.push(
												new DocumentLink(
													range,
													uri.with({
	...
	provideTextDocumentContent(
		...
		return (
			this.mapDocumentCache.get(uri) ??
			this.getDocumentContentAndLinks(uri)
		)?.text;
	}

	private getMapReferenceAtLocation(
	...
	provideDefinition(
		...
		const { references, declarations } =
			this.getDocumentContentAndLinks(document.uri) ?? {};
		const referencedEntries = references?.findAll(position);
		if (referencedEntries) {
			return referencedEntries
				.map(([range, value]) =>
					value instanceof MapId
						? new Location(
								getUriForMap(value),
								new Range(0, 0, 0, 0),
							)
						: value instanceof MapReference
							? new Location(
									getUriForMap(value.mapId),
									new Range(0, 0, 0, 0),
								)
							: undefined,
				)
				...
				.map(([range, value]) =>
					value instanceof MapId
						...
						: value instanceof MapReference
							...
							: value instanceof MapProperty && value.map
								? new Location(
										getUriForMap(value.map.mapId),
										new Range(0, 0, 0, 0),
									)
								: undefined,
				)
		...
		return (
			mapReference && getReferenceLocationForMapName(mapReference.mapId)
		);
	}
	...
	provideReferences(
		...
		for (const ref of map.referencedBy) {
			if (ref.kind === "ic") {
				const location = getScriptSourceLocation(
					ref.entry.getReferenceLocation("source"),
					openedLog?.sources,
				);
				if (location) locations.push(location);
			} else if (ref.kind === "property") {
				let location = getReferenceLocationForMapPropertyType(
					ref.property,
				);
				if (location)
					location = getScriptSourceLocation(
						location,
						openedLog?.sources,
					);
				if (location) locations.push(location);
			} else if (ref.kind === "map") {
				let location = getReferenceLocationForBaseMapInExtendsClause(
					mapId,
					ref.map,
				);
				if (location)
	...
	provideDocumentLinks(
		...
		return (
			this.mapDocumentCache.get(document.uri) ??
			this.getDocumentContentAndLinks(document.uri)
		)?.links;
	}
}
...
function formatMapName(mapId: MapId) {
	return `${formatAddress(mapId.address)}${
		mapId.index ? `_${mapId.index}` : ""
	}`;
}

function formatMapPropertyName({ name }: MapProperty) {
	return name instanceof SymbolName
		? name.toString()
		: /^\d+$/.test(name)
			? name
			: /^[a-z$_][a-z$_\d]*$/i.test(name)
				? name
				: JSON.stringify(name);
	// const symbolMatch =
...
function getCharacterEndOfMapPropertyName(
	property: MapProperty,
	start = getCharacterStartOfMapPropertyName(property),
) {
...
function getCharacterEndOfMapPropertyType(
	property: MapProperty,
	start: number,
) {
...
function getCharacterEndOfMapName(
	mapId: MapId,
	start = getCharacterStartOfMapName(),
) {
	return start + mapId.toString().length;
}

function getCharacterStartOfBaseMapInExtendsClause({
	mapId,
	map,
}: MapReference) {
...
function getReferenceLocationForMapName(mapId: MapId) {
	...
	const end = new Position(
		line,
		getCharacterEndOfMapName(mapId, start.character),
	);
	return new Location(getUriForMap(mapId), new Range(start, end));
}

function getReferenceLocationForBaseMapInExtendsClause(
	mapId: MapId,
	mapRef: MapReference,
) {
	if (resolveBaseMap(mapRef.map)?.mapId.equals(mapId)) {
		...
		const start = new Position(
			line,
			getCharacterStartOfBaseMapInExtendsClause(mapRef),
		);
		const end = new Position(
			line,
			getCharacterEndOfBaseMapInExtendsClause(mapId, start.character),
		);
...
function getReferenceLocationForMapPropertyName(property: MapProperty) {
	if (property.map) {
		...
		const start = new Position(
			line,
			getCharacterStartOfMapPropertyName(property),
		);
		const end = new Position(
			line,
			getCharacterEndOfMapPropertyName(property, start.character),
		);
		return new Location(
			getUriForMap(property.map.mapId),
			new Range(start, end),
		);
	}
}
...
function getReferenceLocationForMapPropertyType(property: MapProperty) {
	if (property.map) {
		...
		const start = new Position(
			line,
			getCharacterStartOfMapPropertyType(property),
		);
		const end = new Position(
			line,
			getCharacterEndOfMapPropertyType(property, start.character),
		);
		return new Location(
			getUriForMap(property.map.mapId),
			new Range(start, end),
...
export async function showMapAsReference(
	mapIds: MapId[],
	uri: Uri,
	position: Position,
) {
...
export function activateMapTextDocumentContentProvider(
	context: ExtensionContext,
) {
	provider = new MapDocumentContentProvider();
	return Disposable.from(
		workspace.registerTextDocumentContentProvider(
			constants.schemes.map,
			provider,
		),
...
function formatTimeRelativeTo(time: TimeTicks, startTime: TimeTicks) {
	...
	return `${negative ? "-" : ""}${hours
		.toString(10)
		.padStart(2, "0")}:${minutes.toString(10).padStart(2, "0")}:${seconds
		.toString(10)
