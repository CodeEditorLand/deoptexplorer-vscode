...
${from(entry.updates).select(
	(update) =>
		markdown.trusted`| ${formatIcState(update.oldState)} | ${formatIcState(update.newState)} | ${update.key} | ${
			update.map
				? markdown.trusted`[\`${new MarkdownString(update.mapId.toString()).trust()}\`](${new CommandUri(
						commands.maps.showMap,
						[
