...
import { MarkdownString, markdown } from "#core/markdown.js";
...
import type { MapEntry } from "../model/mapEntry";
...
function getLastPropertySource(map: MapEntry) {
	...
	if (source) {
		...
		return uri
			? markdown.trusted` | [${source.functionName}](${uri.with({
...
|:-|:-|:-|:-|:-|
${from(entry.updates).select(
	(update) =>
		markdown.trusted`| ${formatIcState(update.oldState)} | ${formatIcState(
			update.newState,
		)} | ${update.key} | ${
			update.map
				? markdown.trusted`[\`${new MarkdownString(
						update.mapId.toString(),
					).trust()}\`](${new CommandUri(commands.maps.showMap, [
						[update.mapId.toString()],
