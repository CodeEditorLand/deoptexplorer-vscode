...
import type { DeoptEntry, DeoptEntryUpdate } from "#deoptigate/deoptEntry.js";
import {
	DeoptimizeKind,
...
function formatSummaryForDeoptEntryUpdate(
	...
	switch (update.bailoutType) {
		...
		default:
			return `'${formatDeoptimizeKind(
				update.bailoutType,
			)}' bailout${getDeoptReason(update)}`;
	}
}

function* getHoverMessageForDeoptEntry(entry: DeoptEntry) {
	yield markdown.code("text")`${formatSummaryForDeoptEntryUpdate(
		from(entry.updates).minBy((update) => update.bailoutType),
	)}`;
	yield markdown.table(
		...
		from(entry.updates)
			...
			.select((update) => [
				/* Timestamp */ formatMillisecondsHighPrecision(
					update.timestamp.sinceOrigin().inMillisecondsF(),
				),
