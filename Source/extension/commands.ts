...
import {
	...
	setShowLineTicks,
	setShowMaps,
	setShowNativeCodeProfileNodes,
	setShowNodeJsProfileNodes,
	setShowNodeModulesProfileNodes,
	setShowProfile,
	setShowProfileJustMyCode,
	setSortDeopts,
	setSortICs,
	setSortMaps,
	setSortProfile,
...
declare global {
	interface KnownCommands {
		...
		[constants.commands.log.removeRecent]: (
			context: ContextCommandHandler,
		) => void;
		[constants.commands.functions.showFunctionHistory]: (
		...
		[constants.commands.maps.showMap]: (
			mapIds: string[],
			file: CanonicalUri,
			line: number,
			character: number,
		...
		[constants.commands.profile.showLineTickDecorationsForNode]: (
			context: ContextCommandHandler,
		) => void;
...
export function activateCommands(context: ExtensionContext) {
	return Disposable.from(
		...
		typeSafeRegisterCommand(
			constants.commands.log.clearRecent,
			async () => {
		...
		typeSafeRegisterCommand(
			constants.commands.log.removeRecent,
			async (context: ContextCommandHandler) => {
				if (context instanceof ContextCommandHandler) {
					await context.onCommand(
						constants.commands.log.removeRecent,
					);
				}
			},
		...
		// ICs
		typeSafeRegisterCommand(
			constants.commands.ics.sortByLocation,
			async () => {
				await setSortICs(constants.SortICs.ByLocation);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.ics.sortByState,
			async () => {
				await setSortICs(constants.SortICs.ByState);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.ics.showStateMegamorphic,
			async () => {
				await setShowICStates(
					showICStates.add(constants.ShowICStates.Megamorphic),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.ics.showStatePolymorphic,
			async () => {
				await setShowICStates(
					showICStates.add(constants.ShowICStates.Polymorphic),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.ics.showStateMonomorphic,
			async () => {
				await setShowICStates(
					showICStates.add(constants.ShowICStates.Monomorphic),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.ics.showStateOther,
			async () => {
				await setShowICStates(
					showICStates.add(constants.ShowICStates.Other),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.ics.hideStateMegamorphic,
			async () => {
				await setShowICStates(
					showICStates.delete(constants.ShowICStates.Megamorphic),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.ics.hideStatePolymorphic,
			async () => {
				await setShowICStates(
					showICStates.delete(constants.ShowICStates.Polymorphic),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.ics.hideStateMonomorphic,
			async () => {
				await setShowICStates(
					showICStates.delete(constants.ShowICStates.Monomorphic),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.ics.hideStateOther,
			async () => {
				await setShowICStates(
					showICStates.delete(constants.ShowICStates.Other),
				);
			},
		),
		...
		// Deopts
		typeSafeRegisterCommand(
			constants.commands.deopts.sortByLocation,
			async () => {
				await setSortDeopts(constants.SortDeopts.ByLocation);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.deopts.sortByKind,
			async () => {
				await setSortDeopts(constants.SortDeopts.ByKind);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.deopts.sortByCount,
			async () => {
				await setSortDeopts(constants.SortDeopts.ByCount);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.deopts.groupByFile,
			async () => {
				await setGroupDeopts(
					groupDeopts.add(constants.GroupDeopts.ByFile),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.deopts.groupByFunction,
			async () => {
				await setGroupDeopts(
					groupDeopts.add(constants.GroupDeopts.ByFunction),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.deopts.groupByKind,
			async () => {
				await setGroupDeopts(
					groupDeopts.add(constants.GroupDeopts.ByKind),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.deopts.ungroupByFile,
			async () => {
				await setGroupDeopts(
					groupDeopts.delete(constants.GroupDeopts.ByFile),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.deopts.ungroupByFunction,
			async () => {
				await setGroupDeopts(
					groupDeopts.delete(constants.GroupDeopts.ByFunction),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.deopts.ungroupByKind,
			async () => {
				await setGroupDeopts(
					groupDeopts.delete(constants.GroupDeopts.ByKind),
				);
			},
		),
		...
		// Maps
		typeSafeRegisterCommand(
			constants.commands.maps.sortByName,
			async () => {
				await setSortMaps(constants.MapSortMode.ByName);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.sortByCount,
			async () => {
				await setSortMaps(constants.MapSortMode.ByCount);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.showUnreferenced,
			async () => {
				await setShowMaps(
					showMaps.add(constants.ShowMaps.Unreferenced),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.hideUnreferenced,
			async () => {
				await setShowMaps(
					showMaps.delete(constants.ShowMaps.Unreferenced),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.showNonUserCode,
			async () => {
				await setShowMaps(showMaps.add(constants.ShowMaps.NonUserCode));
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.hideNonUserCode,
			async () => {
				await setShowMaps(
					showMaps.delete(constants.ShowMaps.NonUserCode),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.showTransitions,
			async () => {
				await setShowMaps(showMaps.add(constants.ShowMaps.Transitions));
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.hideTransitions,
			async () => {
				await setShowMaps(
					showMaps.delete(constants.ShowMaps.Transitions),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.groupByFile,
			async () => {
				await setGroupMaps(groupMaps.add(constants.GroupMaps.ByFile));
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.groupByFunction,
			async () => {
				await setGroupMaps(
					groupMaps.add(constants.GroupMaps.ByFunction),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.ungroupByFile,
			async () => {
				await setGroupMaps(
					groupMaps.delete(constants.GroupMaps.ByFile),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.maps.ungroupByFunction,
			async () => {
				await setGroupMaps(
					groupMaps.delete(constants.GroupMaps.ByFunction),
				);
			},
		),
		...
		// Decorations
		typeSafeRegisterCommand(
			constants.commands.decorations.showDeopts,
			async () => {
				await setShowDecorations(
					showDecorations.add(constants.ShowDecorations.Deopts),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.decorations.hideDeopts,
			async () => {
				await setShowDecorations(
					showDecorations.delete(constants.ShowDecorations.Deopts),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.decorations.showICs,
			async () => {
				await setShowDecorations(
					showDecorations.add(constants.ShowDecorations.ICs),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.decorations.hideICs,
			async () => {
				await setShowDecorations(
					showDecorations.delete(constants.ShowDecorations.ICs),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.decorations.showFunctionStates,
			async () => {
				await setShowDecorations(
					showDecorations.add(constants.ShowDecorations.Functions),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.decorations.hideFunctionStates,
			async () => {
				await setShowDecorations(
					showDecorations.delete(constants.ShowDecorations.Functions),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.decorations.showProfiler,
			async () => {
				await setShowDecorations(
					showDecorations.add(constants.ShowDecorations.Profiler),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.decorations.hideProfiler,
			async () => {
				await setShowDecorations(
					showDecorations.delete(constants.ShowDecorations.Profiler),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.decorations.showLineTicks,
			async () => {
				await setShowDecorations(
					showDecorations.add(constants.ShowDecorations.LineTicks),
				);
			},
		),
		typeSafeRegisterCommand(
			constants.commands.decorations.hideLineTicks,
			async () => {
				await setShowDecorations(
		...
		typeSafeRegisterCommand(
			constants.commands.decorations.hideAll,
			async () => {
		...
		typeSafeRegisterCommand(
			...
			async (context) => {
				if (context instanceof ContextCommandHandler) {
					await context.onCommand(
						constants.commands.profile
							.showLineTickDecorationsForNode,
					);
				}
			},
		),
		...
		// Export CPU profile
		typeSafeRegisterCommand(
			constants.commands.profile.exportProfile,
			async () => {
				if (openedFile && openedLog) {
					...
					if (result) {
						const rawProfile = openedLog.profile.getJSONProfile();
						// rawProfile = ProfileTrimmer.trimProfile(openedLog.rawProfile, { trimUnused: true });
						// rawProfile = SourceMapper.mapProfile(rawProfile);
						let s = "";
						s += `{\n`;
						s += `  "startTime": ${JSON.stringify(
							rawProfile.startTime,
						)},\n`;
						s += `  "endTime": ${JSON.stringify(
							rawProfile.endTime,
						)},\n`;
						s += `  "samples": ${JSON.stringify(
							rawProfile.samples,
						)},\n`;
						s += `  "timeDeltas": ${JSON.stringify(
							rawProfile.timeDeltas,
						)},\n`;
