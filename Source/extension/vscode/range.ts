...
export function tryParseTrailingRange(
	...
	if (match?.groups) {
		...
		const start = new Position(
			Math.max(0, Number.parseInt(line, 10) - 1),
			column ? Math.max(0, Number.parseInt(column, 10) - 1) : 0,
		);
		...
		const end = endLine
			? new Position(
					(endLineNumber = Math.max(
						0,
						Number.parseInt(endLine, 10) - 1,
					)),
					endColumn
						? Math.max(0, Number.parseInt(endColumn, 10) - 1)
...
export function tryParseRange(text: string) {
	...
	if (match?.groups) {
		...
		const end = endLine
			? new Position(
					(endLineNumber = Number.parseInt(endLine, 10)),
					endColumn
						? Number.parseInt(endColumn, 10)
						: endLineNumber === start.line
							? start.character
							: 0,
				)
