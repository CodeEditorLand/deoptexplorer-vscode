...
import {
	...
	RangeSerializer,
	formatRange,
	tryParseTrailingRange,
} from "./range";
import {
	type FormatUriOptions,
	UNKNOWN_URI,
	formatUri,
	isPathOrUriString,
...
export function formatLocationMarkdown(
	...
	const link =
		linkInclude === "none"
			...
			: formatUri(
					linkLocation.uri.with({
						fragment: formatRange(location.range, {
							include: linkInclude,
							delimiter: ",",
							prefix: false,
						}),
...
export const LocationEqualer: Equaler<Location> = Equaler.create(
	(x, y) =>
		equateNullable(x.uri, y.uri, UriEqualer) &&
		equateNullable(x.range, y.range, RangeEqualer),
	(x) =>
		hashNullable(x.uri, UriEqualer) ^ hashNullable(x.range, RangeEqualer),
);
...
export const LocationComparer: Comparer<Location> = Comparer.create(
	(x, y) =>
		compareNullable(x.uri, y.uri, UriComparer) ||
		compareNullable(x.range, y.range, RangeComparer),
);
