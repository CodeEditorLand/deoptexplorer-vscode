...
import type { CommandArgumentValue } from "../types";
...
export class CommandUri {
	...
	toString() {
		return (
			this._uriString ??
			(this._uriString = `command:${this.command}${
				this.commandArgument !== undefined
					? `?${CommandUri.encodeCommandArgument(
	 ...
	 */
	static encodeCommandArgument(
		commandArgument: CommandArgumentValue,
	): string {
		return commandArgument !== undefined
			? encodeURIComponent(JSON.stringify(serialize(commandArgument)))
			: "";
	}
	...
	static decodeCommandArgument(commandArgumentString: string) {
		return commandArgumentString
			? (deserialize(
					JSON.parse(decodeURIComponent(commandArgumentString)),
				) as CommandArgumentValue)
			: undefined;
	}
