...
import type { AsCommandArgumentValue } from "../types";
...
declare global {
	...
	interface KnownCommands {
		...
		"vscode.openFolder": (
			uri?: vscode.Uri,
			options?:
				| { forceNewWindow?: boolean; noRecentEntry?: boolean }
				| boolean,
		) => void;
...
type MapCommandParameters<A extends any[]> = Extract<
	{
		[P in keyof A]: P extends `${number}`
			? AsCommandArgumentValue<A[P]>
			: A[P];
	},
	any[]
>;
...
type ValidKnownCommands2<T extends KnownCommands> = {
	[P in Extract<keyof T, KnownCommandNames>]: (
		...args: MapCommandParameters<KnownCommandParameters<P>>
	) => any;
};
...
export type TypeSafeCommand<K extends KnownCommandNames = KnownCommandNames> =
	K extends KnownCommandNames
		? {
				...
				? { arguments?: [] }
				: [] extends KnownCommands[K]
					? { arguments?: KnownCommandParameters<K> }
					: { arguments: KnownCommandParameters<K> })
		: never;
 ...
 */
export function typeSafeRegisterCommand<K extends KnownCommandNames, T>(
	command: K,
	callback: (
		this: T,
		...args: KnownCommandParameters<K>
...
export function typeSafeRegisterCommand<K extends KnownCommandNames>(
	...
	return vscode.commands.registerCommand(command, (...args: any[]) =>
		callback.apply(thisArg, args.map(deserializeArg) as any),
	);
}
 ...
 */
export function typeSafeExecuteCommand<K extends KnownCommandNames>(
	commandName: K,
	...rest: KnownCommandParameters<K>
): KnownCommandReturnType<K> {
	return vscode.commands.executeCommand<any>(
		commandName,
		...rest.map(serializeArg),
	);
}
 ...
 */
export function typeSafeCommand<K extends KnownCommandNames>(
	command: TypeSafeCommand<K>,
) {
	return command;
}
