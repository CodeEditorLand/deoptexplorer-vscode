...
export function renderFunctionsByTotalTime(log: LogFile, topCount: number) {
    ...
    <ol>${
        from(view.head.children)
            ...
            .select(({ node, functionName, entry }) => html`<li>${entry?.referenceLocation ? renderLinkToFile(functionName.name, entry.referenceLocation, { linkSources: log.sources }) : functionName.name} (${formatMilliseconds(node.totalTime)}) [${functionName.state ? formatFunctionState(functionName.state) : "External"}]</li>`)
