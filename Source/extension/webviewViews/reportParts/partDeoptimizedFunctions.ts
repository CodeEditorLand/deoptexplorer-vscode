...
export function renderDeoptimizedFunctions(log: LogFile, topCount: number) {
	return html` <section style="margin-right:10px;">
		<h2>Top ${topCount} Deoptimized Functions</h2>
		<summary>
			Optimized functions that have been deoptimized multiple times.
		</summary>
		<ol>
			${from(log.files)
				.selectMany(([file, fileEntry]) =>
					from(fileEntry.functions)
						...
						.select((func) => ({
							func,
							file,
							optimizedCount: from(func.updates).count((update) =>
				...
				.select(
					({ func, file }) =>
						html` <li>
							${renderLinkToFile(
								func.functionName,
								func.pickReferenceLocation(file),
								{ linkSources: log.sources }
							)}
							(${from(func.updates).count((update) =>
								isOptimizedFunctionState(update.state)
							)})
						</li>`
				)
