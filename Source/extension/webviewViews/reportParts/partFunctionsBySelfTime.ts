    ...
    <ol>${from(view.head.children)
		...
		.where(
			({ functionName }) =>
				...
				isOpenableScriptUri(functionName.filePosition.uri, log.sources),
		)
		...
		.select(({ node, functionName }) => ({
			node,
			functionName,
			entry: log.findFunctionEntryByFunctionName(functionName),
		}))
		.select(
		...
		.defaultIfEmpty(html`<em>none found</em>`)}</ol>
    </section>
    `;
}
