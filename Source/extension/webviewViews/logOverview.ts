...
import {
	type Slice,
	pieChart,
	pieChartStyleResource,
} from "#chromium/pieChart.js";
import { html } from "#core/html.js";
import * as constants from "../constants";
import {
	formatMemory,
	formatMemoryHighPrecision,
	...
	private refreshWebview() {
		...
		if (this._openedLog) {
			...
			const timeSlices: Slice[] = [
				...
				{
					value: profile.totalIdleTime.inMillisecondsF(),
					title: "Idle",
					color: colors[1],
				},
				{
					value: profile.totalGcTime.inMillisecondsF(),
					title: "GC",
					color: colors[2],
				},
				{
					value: profile.totalFunctionTime.inMillisecondsF(),
					title: "Scripting",
					color: colors[3],
				},
			];
			...
			const entryCategories = from(
				this._openedLog.memory.entrySizes.values(),
			)
				.orderBy((c) => c.name)
				.toArray();
			...
			memorySlices.push({
				value: this._openedLog.memory.size - totalEntrySize,
				title: "Other",
				color: getColor(4 + memorySlices.length),
			});
			...
			webview.html = html`
                ...
                <body>
                    <div class="message">
                        <p>V8 Version: ${
							this._openedLog.version
						} (${this._openedLog.version.toFullString()})</p>
                        <p>Execution Time: ${formatMillisecondsShort(
							this._openedLog.profile.duration.inMillisecondsF(),
						)}</p>
                        <p>Memory Usage: ${formatMemoryHighPrecision(
							this._openedLog.memory.size,
						)}</p>
                        <p style="margin-top: 12px; margin-bottom: 12px;"><a title="Show Report" href="${new CommandUri(
							constants.commands.log.showReport,
						)}">Show Report</a></p>
                        <p><b>Time</b></p>
                        <p>${pieChart(timeSlices, {
							size: 150,
							formatter: formatMillisecondsShort,
							cutout: true,
							legend: true,
						})}</p>
                        ${
...
export function activateLogOverviewWebview(context: ExtensionContext) {
	...
	return Disposable.from(
		...
		events.onWillOpenLogFile(({ uri }) => {
			overviewProvider.openingLog(uri);
		}),
		events.onDidOpenLogFile(({ uri, log }) => {
			overviewProvider.openLog(uri, log);
		}),
		events.onDidCloseLogFile(() => {
			overviewProvider.closeLog();
		}),
	);
}
