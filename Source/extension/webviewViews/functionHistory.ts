...
<body>
    ...
    <ul>
        <li>Location: ${
			entry.referenceLocation
				? renderLinkToFile(
						`${path.basename(entry.referenceLocation.uri.fsPath)}:${
							entry.referenceLocation.range.start.line + 1
						}:${entry.referenceLocation.range.start.character + 1}`,
						entry.referenceLocation,
						{
							title: entry.referenceLocation.uri.fsPath,
    ...
    ${from(entry.timeline).select(
		(event) => html`
        <tr valign="top">
            <td align="right">${formatMillisecondsHighPrecision(
				event.timestamp.sinceOrigin().inMillisecondsF(),
			)}</td>
            <td>${
				event.event === "created"
					? "Created"
					: event.event === "updated"
						...
						: event.event === "moved"
							...
							: event.event === "deleted"
								? "Deleted"
								: event.event === "sfi-moved"
									? "SFI Moved"
									: event.event === "deopt"
										? renderLinkToFile(
												`${formatDeoptimizeKind(
													event.update.bailoutType,
												)} Deopt`,
												event.entry.referenceLocation,
												{
            ...
            <td>${
				event.event === "created" || event.event === "updated"
					? html`
                    Type: ${event.type}<br>
...
export function getUriForFunctionEntry(entry: FunctionEntry) {
	return Uri.parse(
		`${constants.schemes.functionHistory}:${encodeURIComponent(
			entry.filePosition.toString(),
		)}/${encodeURIComponent(entry.functionName)}.md`,
	);
}
