				...
				<ul>
					<li>
						...
						${entry.referenceLocation
							? renderLinkToFile(
									`${path.basename(
										entry.referenceLocation.uri.fsPath
									)}:${
										entry.referenceLocation.range.start
											.line + 1
									}:${
										entry.referenceLocation.range.start
											.character + 1
								...
								<tr valign="top">
									...
									<td>
										...
										event.event === "updated"
											...
											: event.event === "deopt"
											? html` Reason:
											  ${event.update.deoptReason}`
											: event.event === "ic"
											? html` Key: ${event.update.key}<br />
													Old:
													${formatIcState(
														event.update.oldState
													)}<br />
													New:
													${formatIcState(
														event.update.newState
													)}`
											: assert(false)}
									</td>
								</tr>
