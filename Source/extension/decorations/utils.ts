...
import type { MatchingKeys } from "@esfx/type-model";
import {
...
export type DecorationKeys = MatchingKeys<
	typeof constants.colors,
	{ background: string }
>;

export function createBaseDecorationType(
	key: DecorationKeys,
...
export function getTextEditors(kind: "visible" | "active" | "none") {
	const editors = new Set<TextEditor>(
		kind === "visible" ? window.visibleTextEditors : undefined,
	);
	if (kind === "active" && window.activeTextEditor)
		editors.add(window.activeTextEditor);
