...
export class DeoptDecorations {
	...
	private _onDidShowDecorationsChange() {
		this.showDecorations = showDecorations.has(ShowDecorations.Deopts)
			? "visible"
			: "none";
	}
		...
		for (const editor of getTextEditors(this._showDecorations)) {
			...
			if (deopts.length > this._maxDecorations) {
				window.showWarningMessage(
					`There are too many deoptimizations in this file. Only the first ${this._maxDecorations} deoptimizations will be shown.`,
				);
			}

			const decorations = from(deopts)
				.take(this._maxDecorations)
				.select((deopt) => ({
					deopt,
					worst: from(deopt.updates).maxBy(
						fn.property("bailoutType"),
					),
				}))
				.select(({ deopt, worst }) =>
					worst ? { deopt, worst } : undefined,
				)
				.filterDefined()
				.select(({ deopt, worst }) => ({
					deopt,
					worst,
					type: this.getDecorationForBailoutType(worst.bailoutType),
				}))
				.select(({ deopt, worst, type }) =>
					type ? { deopt, worst, type } : undefined,
				)
				.filterDefined()
				.select(({ deopt, worst, type }) => ({
					deopt,
					worst,
					type,
					range: deopt.pickReferenceLocation(fileUri)?.range,
				}))
				.select(({ deopt, worst, type, range }) =>
					range ? { deopt, worst, type, range } : undefined,
				)
				.filterDefined()
				.groupBy(
					({ type }) => type,
	...
	private getDecorationForBailoutType(
		type: DeoptimizeKind,
