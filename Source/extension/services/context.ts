...
import type { ImmutableEnumSet } from "#core/collections/enumSet.js";
import {
	GroupDeopts,
	GroupMaps,
	type LogStatus,
	...
	if (!showICStates.equals(value)) {
		...
		if (currentContext) {
			await Promise.all([
				...
				setContext(
					contextKeys.ics.show.state.megamorphic,
					showICStates.has(ShowICStates.Megamorphic),
				),
				setContext(
					contextKeys.ics.show.state.polymorphic,
					showICStates.has(ShowICStates.Polymorphic),
				),
				setContext(
					contextKeys.ics.show.state.monomorphic,
					showICStates.has(ShowICStates.Monomorphic),
				),
				setContext(
					contextKeys.ics.show.state.other,
	...
	if (!groupDeopts.equals(value)) {
		...
		if (currentContext) {
			await Promise.all([
				...
				setContext(
					contextKeys.deopts.groupByFile,
					groupDeopts.has(GroupDeopts.ByFile),
				),
				setContext(
					contextKeys.deopts.groupByFunction,
					groupDeopts.has(GroupDeopts.ByFunction),
				),
				setContext(
					contextKeys.deopts.groupByKind,
	...
	if (!groupMaps.equals(value)) {
		...
		if (currentContext) {
			await Promise.all([
				...
				setContext(
					contextKeys.maps.groupByFile,
					groupMaps.has(GroupMaps.ByFile),
				),
				setContext(
					contextKeys.maps.groupByFunction,
	...
	if (!showMaps.equals(value)) {
		...
		if (currentContext) {
			await Promise.all([
				...
				setContext(
					contextKeys.maps.showUnreferenced,
					showMaps.has(ShowMaps.Unreferenced),
				),
				setContext(
					contextKeys.maps.showNonUserCode,
					showMaps.has(ShowMaps.NonUserCode),
				),
				setContext(
					contextKeys.maps.showTransitions,
...
export async function setShowDecorations(
	value: ImmutableEnumSet<ShowDecorations>,
) {
		...
		if (currentContext) {
			await Promise.all([
				setContext(
					...
					showDecorations.has(ShowDecorations.Deopts),
				),
				setContext(
					contextKeys.decorations.showICs,
					showDecorations.has(ShowDecorations.ICs),
				),
				setContext(
					contextKeys.decorations.showFunctionState,
					showDecorations.has(ShowDecorations.Functions),
				),
				setContext(
					contextKeys.decorations.showProfiler,
					showDecorations.has(ShowDecorations.Profiler),
				),
				setContext(
					contextKeys.decorations.showLineTicks,
