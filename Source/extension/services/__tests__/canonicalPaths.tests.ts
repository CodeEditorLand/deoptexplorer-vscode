...
describe("canonicalPaths", () => {
	describe("getCanonicalUri", () => {
		...
		it("for file URI when exists", () => {
			jest.resetAllMocks();
			const realpathNative = fs.realpathSync.native as jest.MockedFn<
				typeof fs.realpathSync.native
			>;
			realpathNative.mockImplementationOnce((file) => {
				if (file.toString().replaceAll("\\", "/") === "/a/b/../C")
					return "/a/c";
				throw new Error();
			});
			...
			expect(canonicalUri.toString(/*skipEncoding*/ true)).toBe(
				"file:///a/c"
			);
		});
		...
		it("for file URI when not exists", () => {
			...
			const realpathNative = fs.realpathSync.native as jest.MockedFn<
				typeof fs.realpathSync.native
			>;
			realpathNative.mockImplementationOnce(() => {
				throw new Error();
			});
