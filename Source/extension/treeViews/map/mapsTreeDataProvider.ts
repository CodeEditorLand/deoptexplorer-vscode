...
import { type Query, from } from "@esfx/iter-query";
import {
	Location,
	type Position,
	SymbolKind,
...
import { type MarkdownString, markdown } from "#core/markdown.js";
import { UriComparer, UriEqualer, uriBasename } from "#core/uri.js";
import {
	compareNullable,
	equateNullable,
	getNullableComparer,
...
import { GroupingNode, type GroupingOptions } from "../common/groupingNode";
...
export class MapsTreeDataProvider extends BaseNodeProvider {
	private _groupByConstructor: GroupingOptions<
		MapNode,
		MapConstructorKey,
		Map<string, (FunctionEntry | undefined)[]>
	> = {
		...
		context: (nodes) =>
			from(nodes)
				...
				.select(
					({ map }) =>
						[map.constructorName, map.constructorEntry] as const,
				)
				...
				.groupBy(
					...
					(name, group) =>
						[name, from(group).distinct().toArray()] as const,
				)
				.where(
					([, constructorEntries]) => constructorEntries.length > 1,
				)
				.toMap(
					([constructorName]) => constructorName,
		...
		keySelector: ({ map }, ambiguousGroups) =>
			new MapConstructorKey(
				map.constructorName,
				map.constructorEntry,
				map.mapType,
				ambiguousGroups
					?.get(map.constructorName)
					?.indexOf(map.constructorEntry),
			),
		...
		tooltip: (key) => {
			...
			if (key.constructorEntry) {
				...
				lines.push(
					markdown`**file:** ${formatLocationMarkdown(
						key.constructorEntry?.filePosition,
						{
							as: "file",
							relativeTo: relativeTo,
							linkSources: this.log?.sources,
						},
					)}`,
				);
			}
	...
	private _groupByFile: GroupingOptions<MapNode, Uri | undefined> = {
		...
		tooltip: (key) =>
			key
				? markdown`${uriBasename(key)}\n\n**file:** ${formatUriMarkdown(
						key,
						{ as: "file", linkSources: this.log?.sources },
					)}`
				: undefined,
		...
		tooltip: (key) => {
			...
			const file =
				key.file && key.position
					? formatLocationMarkdown(
							new Location(key.file, key.position),
							{
								as: "file",
								relativeTo,
								linkSources: this.log?.sources,
							...
							.select(
								([mapId, map]) =>
									new MapNode(
										this,
										/*parent*/ undefined,
										MapReference.fromMapId(mapId, map),
									),
							)
	...
	private _applyFilters(q: Query<MapNode>) {
		...
		return q
			.through((q) =>
				showUnreferenced
					? q
					: q.where(({ map }) => map.isReferencedByIC()),
			)
			.through((q) =>
				showNonUserCode
					? q
					: q.where(({ map }) => !map.isNonUserCode()),
			)
			.through((q) =>
				showTransitions
					? q
					: q.where(({ map }) => !map.isIntermediateTransition()),
			);
	}

	private _applyOrder(q: Query<MapNode>) {
		return q.orderBy(({ mapId }) => mapId, MapId.comparer);
	}
...
class MapConstructorKey {
	...
	static readonly comparer = Comparer.create<MapConstructorKey>(
		(left, right) =>
			Comparer.defaultComparer.compare(
				left.constructorName,
				right.constructorName,
			) ||
			compareNullable(
				left.constructorEntry?.filePosition,
				right.constructorEntry?.filePosition,
				LocationComparer,
			) ||
			Comparer.defaultComparer.compare(
				left.constructorEntry?.functionName,
				right.constructorEntry?.functionName,
			) ||
	...
	[Equatable.equals](other: unknown) {
		return (
			other instanceof MapConstructorKey &&
			MapConstructorKey.equaler.equals(this, other)
		);
	}
	...
	[Comparable.compareTo](other: unknown) {
		return other instanceof MapConstructorKey
			? MapConstructorKey.comparer.compare(this, other)
			: 0;
	}
}
...
class MapFunctionKey implements Equatable {
	...
	[Equatable.equals](other: unknown): boolean {
		return (
			other instanceof MapFunctionKey &&
			MapFunctionKey.equaler.equals(this, other)
		);
	}
	...
	[Comparable.compareTo](other: unknown): number {
		return other instanceof MapFunctionKey
			? MapFunctionKey.comparer.compare(this, other)
			: 0;
	}
}
