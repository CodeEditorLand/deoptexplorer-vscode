...
import type { MapReference } from "../../model/mapEntry";
...
export class MapNode extends BaseNode {
	...
	protected createTreeItem() {
		...
		return createTreeItem(
			...
			{
				contextValue: "map",
				description: groupByFunction
					? undefined
					: this.map?.getMapSource()?.functionName,
				iconPath: new ThemeIcon(
					(this.map.mapType &&
						mapTypeToThemeIcon[this.map.mapType]) ??
						"symbol-misc",
				),
				command: {
					title: "",
	...
	override resolveTreeItem(
		...
		if (source) {
			treeItem.description = `${source.functionName} (${formatLocation(
				this.map.getMapFilePosition(),
				{ as: "file", include: "none" },
			)})`;
		}
			...
			lines.push(
				...
				markdown`**file:** ${formatLocationMarkdown(
					this.map.getMapFilePosition(),
					{
						as: "file",
						relativeTo,
						linkSources: this.provider.log?.sources,
					},
				)}  \n`,
			);
		}
		...
		lines.push(
			markdown`**ics:** ${count(
				this.map.referencedBy,
				(ref) => ref.kind === "ic",
			)}  \n`,
		);

		const header = formatUriMarkdown(getUriForMap(this.mapId), {
			label: `${this.mapId}`,
			title: `${this.mapId}`,
		});
