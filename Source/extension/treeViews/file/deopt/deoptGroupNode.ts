...
import type { DeoptCategoryNode } from "./deoptCategoryNode";
...
export class DeoptGroupNode extends GroupNode<DeoptEntry> {
	 ...
	 */
	get parent(): DeoptCategoryNode {
		return super.parent as DeoptCategoryNode;
	}
	...
	protected getChildren(): Iterable<BaseNode> {
		return from(this.entries).select(
			(entry) => new DeoptEntryNode(this, entry),
		);
	}
}
