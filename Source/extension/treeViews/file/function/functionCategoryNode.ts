...
export class FunctionCategoryNode extends CategoryNode<FunctionEntry> {
	...
	protected getChildren(): Iterable<GroupNode<FunctionEntry>> {
		...
		return from(this.entries)
			.groupBy((entry) =>
				from(entry.updates).count((update) =>
					isOptimizedFunctionState(update.state),
				) > 1
					? -1
					: from(entry.updates).last()?.state ??
						FunctionState.Compiled,
			)
			.orderBy(
				(group) => group.key,
				(a, b) => getFunctionStateWeight(a) - getFunctionStateWeight(b),
			)
			.select(
				(group) =>
					new FunctionGroupNode(
						this,
						group.key,
						from(group)
							...
							.thenBy(
								(entry) =>
									entry.pickReferenceLocation(this.file)
										.range,
								RangeComparer,
							)
							.toArray(),
