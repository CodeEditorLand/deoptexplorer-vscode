...
export class ProfileTreeDataProvider extends BaseNodeProvider {
	...
	constructor() {
		super(
			() => {
				if (this._log) {
					if (!this._head) {
						const view = this._log.profile.getProfileView(
							this._showAs,
							{
								hideNativeCode:
									this._showJustMyCode ||
									!this._showNativeCodeProfileNodes,
								hideNodeJsCode:
									this._showJustMyCode ||
									!this._showNodeJsProfileNodes,
								hideNodeModulesCode:
									this._showJustMyCode ||
			...
			.toArray(
				(node) =>
					new ProfileNode(
						this,
						node,
						node instanceof HiddenProfileNode
							? node.hiddenNodes
							: undefined,
					),
			);
	...
	private static _getSorter(
		sortBy: constants.ProfileSortMode,
	): (query: Query<ProfileViewNode>) => Iterable<ProfileViewNode> {
		switch (sortBy) {
	...
	private static _bySelfTimeSorter(
		query: Query<ProfileViewNode>,
	): Iterable<ProfileViewNode> {
		return query
			.orderByDescending((node) => node.selfTime)
			.thenBy(
				({ functionName }) => functionName,
				FunctionNameComparer.compare,
			);
	}

	private static _byTotalTimeSorter(
		query: Query<ProfileViewNode>,
	): Iterable<ProfileViewNode> {
		return query
	...
	private static _byNameSorter(
		query: Query<ProfileViewNode>,
	): Iterable<ProfileViewNode> {
		return query
			.orderBy(({ functionName }) => functionName.name)
			.thenBy(
				({ functionName }) => functionName.filePosition,
				(x, y) => compareNullable(x, y, LocationComparer),
			);
	}
}
...
class HiddenProfileNode extends ProfileViewNode {
	...
	constructor(
		...
		let selfTime = 0;
		const children: ProfileViewNode[] = [];
		const hidden: ProfileViewNode[] = [];
	...
	static maybeHide(
		...
		return shouldHide(node)
			? new HiddenProfileNode(node, shouldHide)
			: node;
	}
