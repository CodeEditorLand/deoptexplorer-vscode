...
export class ProfileNode extends BaseNode {
	 ...
	 */
	get provider(): ProfileTreeDataProvider {
		return super.provider as ProfileTreeDataProvider;
	}
		...
		return createTreeItem(functionName.name, collapsibleState, {
			...
			description:
				this.provider.sortBy === constants.ProfileSortMode.BySelfTime
					? `${formatMilliseconds(
							this.node.selfTime,
						)} (${this.node.selfPercent.toFixed(1)}%)`
					: this.provider.sortBy ===
							constants.ProfileSortMode.ByTotalTime
						? `${formatMilliseconds(
								this.node.totalTime,
							)} (${this.node.totalPercent.toFixed(1)}%)`
						: `${formatMilliseconds(
								this.node.selfTime,
							)} self, ${formatMilliseconds(
								this.node.totalTime,
							)} total`,
		});
	}
	...
	private getIconPath() {
		if (
			this.node.entry instanceof DynamicFuncCodeEntry &&
			this.node.entry.state === FunctionState.Inlined
		) {
			...
			item.tooltip = markdown.trusted`${[
				entry.type === "CPP"
					...
					: entry.type === "SHARED_LIB"
						? markdown.trusted.code`${functionName.name}`
						: markdown.trusted.code(
								"javascript",
							)`${functionName.name}`,
				markdown.trusted`---\n`,
				markdown.trusted`**Type:** ${entry.type}${
					inlined ? " (inlined)" : ""
				}  \n`,
				markdown.trusted`**Self time:** ${formatMilliseconds(
					this.node.selfTime,
				)} (${this.node.selfPercent.toFixed(1)}%)  \n`,
				markdown.trusted`**Total time:** ${formatMilliseconds(
					this.node.totalTime,
				)} (${this.node.totalPercent.toFixed(1)}%)  \n`,
				markdown.trusted`**Percent of parent:** ${this.node.parentTotalPercent.toFixed(
					1,
				)}%  \n`,
				location
				...
				this.forNodes?.length
					? [
							...
							this.forNodes.map((node) => [
								...
								markdown.trusted`${formatLocation(
									node.functionName.filePosition,
									{ as: "file" },
								)}  \n`,
							]),
						]
		...
		if (item.command === undefined && location) {
			...
			if (uri) {
				item.command = typeSafeCommand({
					...
					arguments: [
						uri,
						{ preview: true, selection: location.range },
					],
				});
			}
		}
