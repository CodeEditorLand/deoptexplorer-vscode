...
export class ProfileNodeFileDecorationProvider
	implements FileDecorationProvider
{
	private _rootNodeFileDecoration = new FileDecoration(
		/*badge*/ undefined,
		/*tooltip*/ undefined,
...
export function getUriForProfileNode(node: ProfileViewNode) {
	const filePosition =
		node.entry === CodeEntry.gc_entry()
			...
			? "program"
			: node.entry === CodeEntry.root_entry()
			? "root"
			: node.entry === CodeEntry.unresolved_entry()
			? "unresolved"
			: node.entry === CodeEntry.hidden_entry()
			? "hidden"
			: node.entry.filePosition
			? encodeURIComponent(
					JSON.stringify(serialize(node.entry.filePosition))
			  )
			: "";
	const generated =
...
function parseProfileNodeUri(uri: Uri) {
	...
	const filePosition =
		uri.path === "gc"
			...
			? "hidden"
			: uri.path
			? (deserialize(JSON.parse(uri.path)) as Location)
			: undefined;
