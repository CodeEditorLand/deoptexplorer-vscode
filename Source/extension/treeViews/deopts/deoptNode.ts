...
import type { TypeSafeCommand } from "../../vscode/commands";
import { formatLocation } from "../../vscode/location";
import { BaseNode } from "../common/baseNode";
import { createTreeItem } from "../createTreeItem";
import type { DeoptTreeDataProvider } from "./deoptTreeDataProvider";
	...
	override resolveTreeItem(
		...
		for (const update of this.deopt.updates) {
			...
			if (update.bailoutType !== lastBailoutType) {
				bailouts.push(
					markdown`${formatDeoptimizeKind(
						lastBailoutType,
						this.provider.log?.version,
					)} - ${lastDeoptReason || "(unknown)"}  \n`,
				);
				lastBailoutType = update.bailoutType;
				lastDeoptReason = update.deoptReason;
				break;
