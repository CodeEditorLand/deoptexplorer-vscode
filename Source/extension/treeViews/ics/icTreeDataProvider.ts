	...
	private _groupByState: GroupingOptions<IcNode, IcState> = {
		...
		iconPath: (key) =>
			key === IcState.MEGAMORPHIC
				...
				: key === IcState.MONOMORPHIC || key === IcState.PREMONOMORPHIC
				? new ThemeIcon("symbol-constant", new ThemeColor("foreground"))
				: key === IcState.UNINITIALIZED || key === IcState.NO_FEEDBACK
				? new ThemeIcon(
						"question",
