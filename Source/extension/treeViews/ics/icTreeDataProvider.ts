...
import { SymbolKind, ThemeColor, ThemeIcon, type Uri } from "vscode";
...
import { IcState, formatIcState } from "#v8/enums/icState.js";
...
export class IcTreeDataProvider extends BaseNodeProvider {
	private _summarizeStates(elements: readonly IcNode[]) {
		...
		const hitCount = from(elements).sum((n) => n.hitCount);
	...
	private _groupByFile: GroupingOptions<
		IcNode,
		Uri | undefined,
		GroupingNode<IcNode, IcState> | undefined
	> = {
		context: (_, parent) => {
		...
		description: (_, elements, parent) =>
			this.sortBy === constants.SortICs.ByLocation
				? this._summarizeStates(elements)
		...
		tooltip: (key, elements, parent) => {
			...
			const hitCount =
				this.sortBy === constants.SortICs.ByLocation
					? from(elements).sum((n) => n.hitCount)
					: from(elements)
							.selectMany((n) => n.ic.updates)
							.select((u) => u.newState)
							.through(
			...
			lines.push(
				markdown.trusted`**worst state:** ${formatIcState(
					worstState,
				)}  \n`,
			);

			return markdown.trusted`${formatUriMarkdown(key, {
				as: "file",
				skipEncoding: true,
	...
	> = {
		context: (_, parent) =>
			...
			parent.parent instanceof GroupingNode &&
			parent.parent.groupingOptions === this._groupByState
				? parent.parent
				: undefined,
		...
		tooltip: (key, elements, grandparent) => {
			if (key) {
				...
				lines.push(
					markdown`**file:** ${formatLocationMarkdown(key.location, {
						as: "file",
						relativeTo,
						include: "position",
						skipEncoding: true,
				...
				lines.push(
					markdown`**worst state:** ${formatIcState(worstState)}  \n`,
				);
		...
		command: (key) => {
			if (
				!key?.location ||
				this.log?.generatedPaths.has(key.location.uri)
			)
				return;
		...
		sorter: (q) =>
			q
				.select((g) => ({
					g,
					e: from(g.elements).maxBy((e) => e.state)!,
				}))
	...
	private _groupByState: GroupingOptions<IcNode, IcState> = {
		...
		description: (key, elements) =>
			this.sortBy === constants.SortICs.ByLocation
				? this._summarizeHitCount(elements)
				: this._summarizeHitCount(elements, key),

		iconPath: (key) =>
			key === IcState.MEGAMORPHIC
				? new ThemeIcon(
						"warning",
						new ThemeColor("list.errorForeground"),
					)
				: key === IcState.POLYMORPHIC
					? new ThemeIcon(
							"versions",
							new ThemeColor("list.warningForeground"),
						)
					: key === IcState.MONOMORPHIC ||
							key === IcState.PREMONOMORPHIC
						? new ThemeIcon(
								"symbol-constant",
								new ThemeColor("foreground"),
							)
						: key === IcState.UNINITIALIZED ||
								key === IcState.NO_FEEDBACK
							? new ThemeIcon(
									"question",
									new ThemeColor(
		...
		sorter: (q) => q.orderByDescending((g) => g.key),
	};
				...
				if (this._log) {
					if (!this._ics) {
						this._ics = from(this._log.files.values())
							...
							.select(
								(ic) =>
									new IcNode(this, /*parent*/ undefined, ic),
							)
