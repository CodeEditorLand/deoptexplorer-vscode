...
export class IcsTree implements Disposable {
	...
	private updateTreeViewHeader() {
		const sortBy =
			this.provider.sortBy === constants.SortICs.ByLocation
				? `By File/Function/State, Location`
				: this.provider.sortBy === constants.SortICs.ByState
					? `By State/File/Function, State`
					: // this.provider.sortBy === constants.SortICs.ByCount ? `By File/Function/Kind, Count` :
						"";

		const filtered =
			this.provider.showICStates.size > 0 &&
			this.provider.showICStates.size < constants.kCountShowICStates
				? ` (filtered)`
				: ``;
