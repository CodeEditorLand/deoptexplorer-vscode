...
import type { CancellationToken, ProviderResult, TreeItem } from "vscode";
import { assert } from "#core/assert.js";
import { ContextCommandHandler } from "../../commands";
import { type PaginationOptions, paginateNodes } from "../pagination";
...
export abstract class BaseNode extends ContextCommandHandler {
	 ...
	 */
	get visualParent(): BaseNode | undefined {
	...
	get treeItem(): TreeItem {
		return this.ensureTreeItem();
	}

	/**
	 * Gets the children (or a Promise for the children) of this node.
	 */
	get children(): BaseNode[] | Promise<BaseNode[]> {
	...
	ensureChildren(): BaseNode[] | Promise<BaseNode[]> {
		return (this._children ??= Promise.resolve(this.getChildren()).then(
			(children) => this.paginateChildren([...children]),
		));
	}
