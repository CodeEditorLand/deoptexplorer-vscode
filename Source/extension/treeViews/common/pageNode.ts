...
export class PageNode extends BaseNode {
	...
	protected createTreeItem() {
		return createTreeItem(
			this.formatLabel?.(this.start, this.page) ??
				`[${this.start}..${this.start + this.page.length}]`,
			TreeItemCollapsibleState.Collapsed,
			{
				contextValue: this.parent?.treeItem.contextValue
					? `${this.parent.treeItem.contextValue}-page`
					: "page",
	...
	protected getChildren(): Iterable<BaseNode> {
		return from(this.page).select(
			(node) =>
				Object.create(node, {
					_visualParent: { value: this },
				}) as BaseNode,
		);
	}
}
