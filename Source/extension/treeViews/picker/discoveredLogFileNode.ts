...
export class DiscoveredLogFileNode extends BaseNode {
	 ...
	 */
	get provider(): PickerTreeDataProvider {
		return super.provider as PickerTreeDataProvider;
	}

	/**
	 * Gets the parent of the this node.
	 */
	get parent(): RecentLogFilesNode | WorkspaceLogFilesNode {
		return super.parent as RecentLogFilesNode | WorkspaceLogFilesNode;
	}
	...
	async onCommand(commandName: string) {
		if (
			commandName === constants.commands.log.removeRecent &&
			this.recent
		) {
