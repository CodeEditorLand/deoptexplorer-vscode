...
import { FunctionState, formatFunctionState } from "#v8/enums/functionState.js";
...
export class FunctionsTreeDataProvider extends BaseNodeProvider {
	...
	private _applyGroups = (nodes: Query<FunctionNode>) =>
		nodes
			.through((nodes) =>
				nodes.orderBy((e) => getFunctionStateWeight(e.state)),
			)
			.through((nodes) =>
				GroupingNode.groupBy(nodes.toArray(), [
					...
					GroupingNode.createGrouping({
						...
						label: (state) =>
							state === undefined
								...
								: state === -1
									? "Reoptimized"
									: formatFunctionState(state),
						sorter: (q) =>
							q.orderBy((g) => getFunctionStateWeight(g.key)),
					}),
				]),
			);
