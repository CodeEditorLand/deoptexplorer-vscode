...
import type { FileLineTick } from "#v8/tools/types.js";
...
import type { LineTickTreeDataProvider } from "./lineTickTreeDataProvider";
...
export class LineTickNode extends BaseNode {
	...
	get provider(): LineTickTreeDataProvider {
		return super.provider as LineTickTreeDataProvider;
	}
	...
	protected createTreeItem(): TreeItem {
		...
		return createTreeItem(
			...
			TreeItemCollapsibleState.None,
			{
				...
				description: `${formatMilliseconds(
					selfTime,
				)} (${selfPercent.toFixed(1)}%)`,
				command: uri && {
					title: "open",
					command: "vscode.open",
					arguments: [
