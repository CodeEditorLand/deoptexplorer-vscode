...
import {
	type KnownSerializedType,
	type RegisteredSerializer,
...
import type { CodeEntry } from "#v8/tools/codeentry.js";
...
export class FunctionName {
	...
	static parse(text: string) {
		...
		if (match?.groups) {
			...
			let range = tryParseTrailingRange(
				nameAndLocation,
				out_prefixLength,
			);
			if (range) {
			...
			return new FunctionName(
				...
				location,
				type,
				state ? parseFunctionState(state) : undefined,
			);
		}
	...
	[Equatable.equals](other: unknown) {
		return other instanceof FunctionName && this.equals(other);
	}
	[Equatable.hash]() {
		return this.hash();
	}
	[Comparable.compareTo](other: unknown) {
		return other instanceof FunctionName ? this.compareTo(other) : 0;
	}
}

export const FunctionNameEqualer = Equaler.create<FunctionName>(
	(x, y) =>
		equateNullable(x.filePosition, y.filePosition, LocationEqualer) &&
		equateNullable(x.name, y.name),
...
export const FunctionNameComparer = Comparer.create<FunctionName>(
	(x, y) =>
		compareNullable(x.filePosition, y.filePosition, LocationComparer) ||
		compareNullable(x.name, y.name),
);
...
export const FunctionNameSerializer = registerKnownSerializer("FunctionName", {
	...
	serialize: (obj, serialize) => ({
		...
		filePosition:
			obj.filePosition &&
			LocationSerializer.serialize(obj.filePosition, serialize),
	}),
	deserialize: (obj, deserialize) =>
		new FunctionName(
			obj.name,
			obj.filePosition &&
				LocationSerializer.deserialize(obj.filePosition, deserialize),
		),
});

declare global {
	interface KnownSerializers {
		FunctionName: RegisteredSerializer<
			FunctionName,
			{
				$type: "FunctionName";
				name: string;
