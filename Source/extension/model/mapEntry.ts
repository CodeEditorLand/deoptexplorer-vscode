...
import { type Address, formatAddress, parseAddress } from "#core/address.js";
...
import type { IcEntry, IcEntryUpdate } from "#deoptigate/icEntry.js";
...
export class MapId implements ToMarkdownString {
	...
	static readonly equaler = Equaler.create<MapId>(
		(left, right) =>
			left.address === right.address && left.index === right.index,
		(value) =>
			Equaler.combineHashes(
	...
	static tryParse(text: string) {
		...
		const index = fields.index ? Number.parseInt(fields.index, 10) : 0;
	...
	toString() {
		return (this._text ??= this.index
			? `${formatAddress(this.address)}_${this.index}`
...
export class MapEntry {
	...
	get constructorName() {
		return (
			this.constructorEntry?.functionName ?? this._constructorName ?? ""
		);
	}
		...
		if (
			...
			every(
				...
				(update) =>
					...
					update.functionEntry.isNonUserCode(),
			)
		)
			return true;
		if (
...
export class MapEntryUpdate extends EntryBase {
	...
	get functionName() {
		return this.functionEntry?.functionName ?? this._functionName ?? "";
	}
...
export class SymbolName implements Equatable, Comparable {
	...
	[Equatable.equals](other: unknown): boolean {
		return (
			other instanceof SymbolName && SymbolNameEqualer.equals(this, other)
		);
	}
	...
	static tryParse(text: string) {
		try {
			if (text.startsWith("symbol(") && text.endsWith(")")) {
				...
				const hashIndex = text.lastIndexOf("hash ");
				let hash: number | undefined;
				if (hashIndex >= 0) {
					hash = Number.parseInt(
						text.slice(hashIndex + 5).trim(),
						16,
					);
					if (isFinite(hash)) {
...
export const SymbolNameEqualer = Equaler.create<SymbolName>(
	...
	(a) => Equaler.combineHashes(hash(SymbolName), hash(a.name)),
);

export const SymbolNameComparer = Comparer.create<SymbolName>(
	(a, b) =>
		compare(
			typeof a.name === "string" ? 1 : 0,
			typeof b.name === "string" ? 1 : 0,
...
export const PropertyNameEqualer = Equaler.create<string | SymbolName>(
	(a, b) =>
		a === b ||
		(a instanceof SymbolName &&
			b instanceof SymbolName &&
			SymbolNameEqualer.equals(a, b)),
...
export const PropertyNameComparer = Comparer.create<string | SymbolName>(
	(a, b) =>
		a instanceof SymbolName
			? b instanceof SymbolName
				? SymbolNameComparer.compare(a, b)
				: -1
			: b instanceof SymbolName
				? +1
				: compare(a, b),
...
export class MapReferencedByMap {
	...
	constructor(readonly map: MapReference) {}
}
...
export class MapReferencedByMapProperty {
	...
	constructor(readonly property: MapProperty) {}
}
