...
import { type CancellationToken, Location, Position } from "vscode";
...
import { type CanonicalUri, getCanonicalUri } from "../services/canonicalPaths";
...
export class ProfileViewNodeSnapshot {
	...
	get averageSampleDuration() {
		return (
			this.log?.profile.averageSampleDuration ??
		...
		return (this._fileLineTicks = uri
			? this.lineTicks.map(
					({ line, hitCount: hit_count }) =>
						new FileLineTick(uri, line, hit_count),
				)
			: []);
	}

	tryGetMappedLineTicks() {
		return isArray(this._mappedLineTicks)
			? this._mappedLineTicks
			: undefined;
	}
					...
					if (sourceLocation) {
						...
						if (
							...
							mappedLineTicks.push(
								new FileLineTick(
									sourceFile,
									sourceLocation.range.start.line,
									lineTick.hitCount,
								),
							);
							continue;
